...
class FieldSolver
{
   public:
	...
	virtual void update_b_values(Particle dev_particle, const Slice* dev_slice, int Np_sur, double charge, int thread_x, int block_x, int turn,
								 double s) = 0;
   ...
   protected:
	FieldSolver(int input_Nx, int input_Ny, double input_Lx, double input_Ly, int input_Nslice, FieldSolverType input_solverType,
...
class FieldSolverCUDSS : public FieldSolver
{
	...
	*/
   public:
	FieldSolverCUDSS(int input_Nx, int input_Ny, double input_Lx, double input_Ly, int input_Nslice, std::shared_ptr<Aperture> input_aperture);

	~FieldSolverCUDSS() override;

	void initialize() override;
	void update_b_values(Particle dev_particle, const Slice* dev_slice, int Np_sur, double charge, int thread_x, int block_x, int turn,
						 double s) override;
...
class FieldSolverAMGX : public FieldSolver
{
	...
	*/
   public:
	FieldSolverAMGX(int input_Nx, int input_Ny, double input_Lx, double input_Ly, int input_Nslice, std::shared_ptr<Aperture> input_aperture);
...
__global__ void allocate2grid_circle_multi_slice(Particle dev_particle, double* __restrict__ dev_charDensity, const Slice* __restrict__ dev_slice,
												 const MeshMask* __restrict__ dev_meshMask, int Np_sur, int Nslice, int Nx, int Ny, double Lx,
												 double Ly, double charge, double radius_square, int turn, double s);

__global__ void allocate2grid_rectangle_multi_slice(Particle dev_particle, double* __restrict__ dev_charDensity, const Slice* __restrict__ dev_slice,
													const MeshMask* __restrict__ dev_meshMask, int Np_sur, int Nslice, int Nx, int Ny, double Lx,
													double Ly, double charge, double half_width, double half_height, int turn, double s);

__global__ void allocate2grid_ellipse_multi_slice(Particle dev_particle, double* __restrict__ dev_charDensity, const Slice* __restrict__ dev_slice,
												  const MeshMask* __restrict__ dev_meshMask, int Np_sur, int Nslice, int Nx, int Ny, double Lx,
												  double Ly, double charge, double hor_semi_axis, double ver_semi_axis, int turn, double s);

__global__ void cal_electricField(double* __restrict__ dev_potential, double2* __restrict__ dev_electricField,
								  const MeshMask* __restrict__ dev_meshMask, int Nx, int Ny, int Nslice);
...
void generate_meshMask_and_5points_FD_matrix_include_boundary(MeshMask* host_meshMask, double* host_matrix, int Nx, int Ny, double Lx, double Ly,
															  int& nnz, const std::shared_ptr<Aperture>& aperture);

void generate_5points_FD_CSR_matrix_and_meshMask_include_boundary(std::vector<int>& csr_row_ptr, std::vector<int>& csr_col_indices,
																  std::vector<double>& csr_values, MeshMask* host_meshMask, int Nx, int Ny, double Lx,
																  double Ly, int& nnz, const std::shared_ptr<Aperture>& aperture);