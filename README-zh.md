...
先显示主窗口，再自动分阶段准备工具。将项目或束流 JSON 拖入窗口即可打开，替换当前文档前会确认。**工具 → 数据格式转换** 支持预览并选择 OMC3 LHC/TbT SDDS 和 HDF5 数据导出为 CSV/TFS，也支持 CSV/TFS 转回 OMC3 SDDS/HDF5；源文件保持只读。相同函数位于 `PASS.tool.data_conversion`，脚本单独使用 SDDS 时可安装 `".[conversion]"`。已有 GUI 环境需重新执行 GUI 安装命令补充 PyLHC `sdds` 与 `turn-by-turn`。使用方法与格式限制见[GUI 工具说明](docs/source/zh/gui_tools.rst)。
