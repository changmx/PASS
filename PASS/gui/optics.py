...
class OpticsParameters(BaseModel):
    ...
    def betas(self) -> tuple[float, float]:
        if self.mode == "smooth":
            return (self.circumference / (2 * math.pi * self.qx),
                    self.circumference / (2 * math.pi * self.qy))
        return self.beta_x, self.beta_y

    def generate(self) -> tuple[list[TwissPoint], list[str]]:
        ...
        if self.mode == "smooth":
            items, _, _ = generate_smooth_twiss(
                circumference=self.circumference, qx=self.qx, qy=self.qy,
                num_points=self.num_segments + 1,
                alpha_x=self.alpha_x, alpha_y=self.alpha_y,
                dx=self.dx, dpx=self.dpx, muz=muz, dqx=self.dqx, dqy=self.dqy,
        ...
        item = TwissPoint(
            s=self.circumference, s_previous=0.0,
            alpha_x=self.alpha_x, alpha_y=self.alpha_y,
            beta_x=self.beta_x, beta_y=self.beta_y,
            alpha_x_previous=self.alpha_x, alpha_y_previous=self.alpha_y,
            beta_x_previous=self.beta_x, beta_y_previous=self.beta_y,
            dx=self.dx, dpx=self.dpx, dx_previous=self.dx, dpx_previous=self.dpx,
            mu_x=self.qx, mu_y=self.qy, mu_z=muz,
            mu_x_previous=0.0, mu_y_previous=0.0, mu_z_previous=0.0,
            dqx=self.dqx, dqy=self.dqy,
            longitudinal_transfer=self.longitudinal_transfer,
        )
