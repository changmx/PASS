...
from PASS.gui.tool_beam import format_number, hint, create_number_input, output
...
class MagnetSection(QWidget):
    ...
    def __init__(self, kind, owner):
        ...
        self.length = owner.bind(create_number_input(1 if kind == "dipole" else .5))
        form.addRow("有效磁长 L (m)", self.length)
        self.known = QComboBox()
        choices = {
            "dipole": (("磁场 B (T)", "field"), ("弯转半径 ρ (m)", "radius"), ("弯转角 θ (deg)", "angle"), ("积分场 ∫B dl (T·m)", "integrated_field")),
            "quadrupole": (("梯度 G (T/m)", "gradient"), ("归一化 k1 (1/m²)", "k1"), ("积分强度 K1L (1/m)", "k1l"), ("薄透镜 fx (m)", "focal_x"),
                           ("极面场 Bp (Gauss)", "pole_field")),
            "sextupole": (("二阶梯度 G2 (T/m²)", "derivative"), ("归一化 k2 (1/m³)", "kn"), ("积分强度 K2L (1/m²)", "knl"),
                          ("积分梯度 (T/m)", "integrated_derivative"), ("极面场 Bp (Gauss)", "pole_field")),
            "octupole": (("三阶梯度 G3 (T/m³)", "derivative"), ("归一化 k3 (1/m⁴)", "kn"), ("积分强度 K3L (1/m³)", "knl"),
                         ("积分梯度 (T/m²)", "integrated_derivative"), ("极面场 Bp (Gauss)", "pole_field")),
            "solenoid":
            (("轴向场 Bz (Gauss)", "field"), ("归一化 Ks (1/m)", "ks"), ("Larmor 参数 θ (deg)", "angle"), ("积分场 ∫Bz dl (T·m)", "integrated_field")),
        }[kind]
        ...
        self.value = owner.bind(create_number_input(1 if kind == "dipole" else 5, -1e100))
        ...
        fields = {
            "dipole": [("field", "磁场 B (T)", 1), ("radius", "弯转半径 ρ (m)", 1), ("curvature", "曲率 k0 (1/m)", 1), ("angle", "K0L = θ (rad)", 1),
                       ("angle_deg", "弯转角 θ (deg)", 1), ("integrated_field", "积分场 ∫B dl (T·m)", 1)],
            "quadrupole": [("gradient", "梯度 G (T/m)", 1), ("k1", "归一化 k1 (1/m²)", 1), ("k1l", "积分强度 K1L (1/m)", 1),
                           ("integrated_gradient", "积分梯度 ∫G dl (T)", 1), ("focal_x", "薄透镜 fx (m)", 1), ("focal_y", "薄透镜 fy (m)", 1),
                           ("pole_field", "极面场 Bp (Gauss)", 1e4)],
            "solenoid": [("field", "轴向场 Bz (Gauss)", 1e4), ("ks", "PASS Ks (1/m)", 1), ("kappa", "κ = Ks/2 (1/m)", 1),
                         ("angle", "Larmor 参数 θ (rad)", 1), ("integrated_field", "积分场 (T·m)", 1), ("focusing", "聚焦系数 κ² (1/m²)", 1),
                         ("focal", "弱透镜焦距 f (m)", 1)]
        }.get(kind)
    ...
    def calculate(self, brho):
        ...
        if fn is None:
            result = multipole_from(2 if self.kind == "sextupole" else 3, brho, self.length.value(), self.known.currentData(), value,
                                    self.radius.value() * 1e-3)
        else:
            result = fn(brho, self.length.value(), self.known.currentData(), value,
                        **({
                            "radius": self.radius.value() * 1e-3
                        } if self.kind == "quadrupole" else {}))
        ...
        return {
            "magnet": self.kind,
            "inputs": {
                "L_m": self.length.value(),
                "known": self.known.currentText(),
                "value": self.value.value(),
                "signed_brho_Tm": brho,
                "reference_radius_mm": self.radius.value() if hasattr(self, "radius") else None
            },
...
class MagnetPage(PhysicsToolPage):
    ...
    def __init__(self, source=None):
        ...
        self.direct_rigidity = self.bind(create_number_input(3, -1e100))
