...
from PASS.gui.tool_physics_common import PhysicsToolPage, ResultFields, create_integer_input
...
class RFBucketPage(PhysicsToolPage):
    ...
    def __init__(self, source=None):
        ...
        self.voltage = self.bind(create_number_input(100))
        self.harmonic = self.bind(create_integer_input(4, 1))
        self.circumference = self.bind(create_number_input(100))
        self.phase = self.bind(create_number_input(0, -1e9))
        for label, widget in (("RF 幅值 V (kV)", self.voltage), ("RF 谐波数 h", self.harmonic), ("环周长 C (m)", self.circumference), ("同步相位 φs (deg)",
                                                                                                                               self.phase)):
        ...
        self.gamma_t = self.bind(create_number_input(6))
        self.eta = self.bind(create_number_input(-.1, -1e100))
        ...
        self.orbits = self.bind(create_integer_input(5, 0, 20))
        ...
        self.results = ResultFields([("eta", "滑移因子 η", 1), ("delta_max", "δ 半高 (%)", 100), ("energy_half_height_ev", "ΔE/A 半高 (MeV)", 1e-6),
                                     ("phase_width", "相位全宽 (deg)", 180 / math.pi), ("length_width", "z 全宽 (m)", 1), ("time_width", "时间全宽 (μs)", 1e6),
                                     ("area_ev_s", "桶面积 (eV·s/核子)", 1), ("synchrotron_tune", "同步振荡 Qs", 1),
                                     ("synchrotron_frequency", "同步频率 fs (Hz)", 1), ("revolution_frequency", "回旋频率 (MHz)", 1e-6),
                                     ("rf_frequency", "RF 频率 (MHz)", 1e-6), ("energy_gain_ev", "每核子每圈增能 (MeV)", 1e-6)])
    ...
    def recalculate(self):
        ...
        try:
            with np.errstate(over="raise", invalid="raise", divide="raise"):
                ...
                r = calculate_bucket(k,
                                     self.voltage.value() * 1e3, self.harmonic.value(), self.circumference.value(), self.phase.value(),
                                     **({
                                         "eta": self.eta.value()
                                     } if self.eta_mode.currentData() == "eta" else {
                                         "gamma_t": self.gamma_t.value()
                                     }))
                colors = self.prepare_plot()
                for fraction in [1., *np.linspace(.12, .9, self.orbits.value())]:
                    offsets, delta = r.contour(float(fraction))
                    x = np.degrees(offsets +
                                   r.phase_s) if self.x_axis.currentData() == "phase" else -offsets * r.circumference / (2 * np.pi * r.harmonic)
                    ...
                    self.ax.plot(x,
                                 y,
                                 color=colors["accent"] if fraction == 1 else colors["muted"],
                                 lw=1.6 if fraction == 1 else .8,
                                 label="Separatrix" if fraction == 1 else None)
                ...
                self.set_valid({
                    "inputs": {
                        "V_kV": self.voltage.value(),
                        "h": self.harmonic.value(),
                        "C_m": self.circumference.value(),
                        "phi_s_deg": self.phase.value(),
                        "eta_mode": self.eta_mode.currentData(),
                        "eta": r.eta
                    },
    ...
    def export_rows(self):
        ...
        return ("phase_deg", "z_rel_m", "delta_upper", "delta_lower", f"delta_E_upper_{unit}",
                f"delta_E_lower_{unit}"), zip(np.degrees(offsets + r.phase_s), z, delta, -delta, de, -de)
