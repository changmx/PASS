...
class ExciterPage(PhysicsToolPage):
    ...
    def __init__(self, source=None):
        ...
        for key, label, value, scale in (("circumference", "环周长 C (m)", 100, 1), ("voltage", "带符号峰值电压差 V (V)", 1000, 1),
                                         ("gap", "极板间距 d (mm)", 50, .001), ("plate_length", "极板有效长 L (m)", .5, 1)):
        ...
        for key, label, value, scale in (("excite_tune", "中心激励 tune", .47, 1), ("sweep_tune", "扫频 tune 全宽", .02, 1),
                                         ...
                                         ("period", "扫频周期 T (ms)", 1, .001), ("dual_frequency", "双频参数 fd (Hz)", 1000, 1)):
        ...
        self.results = ResultFields([("f0", "回旋频率 f0 (kHz)", .001), ("cf", "中心频率 fc (kHz)", .001), ("width", "扫频全宽 Δf (kHz)", .001),
                                     ("tune", "中心激励 tune", 1), ("sweep_tune", "扫频 tune 全宽", 1), ("amplitude", "带符号基准踢角 A0 (mrad)", 1000),
                                     ("peak", "采样最大 |踢角| (mrad)", 1000), ("rms", "采样 RMS 踢角 (mrad)", 1000), ("field", "基准电场 V/d (V/m)", 1),
                                     ("transit", "过板时间 (ns)", 1e9), ("rate", "采样率 (MHz)", 1e-6), ("resolution", "频谱间隔 (Hz)", 1)])
        self.result_layout.setContentsMargins(0, 0, 0, 0)
        self.result_layout.addWidget(self.results)
        self.result_layout.addWidget(
            hint("预览固定参考状态（dp=px=py=0，速度因子为 1），按所设 z_rel 计算到达相位。"
    ...
    def add_input(self, form, key, label, value, scale=1, minimum=0):
        widget = self.bind(create_number_input(value, minimum))
    ...
    def recalculate(self):
        ...
        try:
            ...
            if kind == "wave":
                ...
                self.ax.plot(time,
                             r.lower_frequency / 1000,
                             color=colors["accent"],
                             label="FM frequency" if settings.mode.startswith("single") else "Lower branch")
            ...
            values = dict(f0=r.frequency_0,
                          ...
                          field=settings.voltage / settings.gap,
                          transit=settings.plate_length / r.velocity,
                          rate=r.sample_rate,
    ...
    def export_rows(self):
        ...
        return ("elapsed_time_s", "arrival_time_s", "kick_rad", "envelope_rad", "am_factor", "fm_lower_Hz",
                "fm_upper_Hz"), zip(r.times, r.arrival_times, r.kick, r.envelope, r.am_factor, r.lower_frequency, r.upper_frequency)
