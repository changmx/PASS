...
def _drift(p, bunch, length, s0, turn):
    ...
    px, py, dp = p.px[bunch_slice], p.py[bunch_slice], p.dp[bunch_slice]
    tag = p.tag[bunch_slice]
    ...
    p.lost_position[bunch_slice][lost] = s0
    if length == 0.0:
        return
    active = tag > 0
...
class Bump(Command):
    ...
    def __init__(self, beam_id, sim, **command_kwargs):
        ...
        self.cmd_name, self.s = kwargs["name"], float(kwargs["s (m)"])
        self.length = float(kwargs.get("length (m)", 0.0))
        self.num_slice = kwargs.get("num slices", 1)
        self.time_mode = kwargs.get("time mode", "particle")
        self.time_offset = float(kwargs.get("time offset (s)", 0.0))
        ...
        self.waveform, self.waveform_bounds = read_bump_waveform(kwargs["waveform file"])
    ...
    def execute_gpu(self, sim):
        ...
        if key not in self._gpu_cache:
            self._gpu_cache[key] = (
                cp.RawKernel(_bump_kernel_source(),
                             "track_bump",
        ...
        reference_time = (beam.reference_program.inverse_integral(float(turn)) if self.time_mode == "reference" and self.enabled else 0.0)
        ...
        h_ref, v_ref = (tuple(np.interp(reference_time, self.waveform[:, 0], self.waveform[:, j])
                              for j in (1, 2)) if self.time_mode == "reference" and self.enabled else (0., 0.))
            ...
            def launch(before, fraction, after, *, repeats=1):
                ...
                if count:
                    kernel(((count + 255) // 256, ), (256, ),
                           (p.x, p.px, p.y, p.py, p.z, p.dp, p.tag, p.lost_position, p.lost_turn, table, outside, np.int32(
                               bunch.start_idx), np.int32(bunch.end_idx), np.int32(len(self.waveform)), np.int32(turn), np.int32(repeats),
                            np.int32(self.enabled and fraction != 0.), np.int32(self.time_mode == "particle"), np.int32(not self._warned),
                            np.float64(before), np.float64(after), np.float64(fraction), np.float64(self.s - self.length + offset_s),
                            np.float64(bunch.t0), np.float64(bunch.beta * const.c), p.real(1. / bunch.gamma**2), np.float64(
                                self.time_offset), np.float64(reference_time), np.float64(self._warning_start), np.float64(
                                    self._warning_end), np.float64(h_ref), np.float64(v_ref)))
    ...
    def _warn_outside(self):
        logger.warning("Bump %s: time outside supplied waveform range; holding nearest endpoint values "
                       "(HKICK [%g, %g] s; VKICK [%g, %g] s)", self.cmd_name, *self.waveform_bounds.ravel())
        self._warned = True
        ...
        for bunch in beam.bunches:
            ...
            def kick(fraction):
                ...
                if self.time_mode == "particle":
                    times = bunch.t0 - p.z[bunch_slice].astype(xp.float64) / (bunch.beta * const.c)
                    times = times[alive] + self.time_offset
                else:
                    times = xp.full(int(alive.sum()), reference_time + self.time_offset)
                if not self._warned and bool(xp.any((times < self._warning_start) | (times > self._warning_end))):
                    self._warn_outside()
                for column, name in ((1, "px"), (2, "py")):
                    values = xp.interp(times, table[:, 0], table[:, column])
                    getattr(p, name)[bunch_slice][alive] += (fraction * values).astype(p.dtype)
...
extern "C" __global__ void track_bump(
    ...
    for (int step = 0; step < repeats; ++step) {
        if (!bump_drift(xi, yi, zi, pxi, pyi, dpi, ti, ls, lt, inv_gamma2, before, s0, turn))
            break;
        s0 += before;
        t0 += before / velocity;
        if (enabled) {
            ...
            if (out && check_warning) {
                ...
                if ((threadIdx.x & 31) == __ffs(mask) - 1)
                    atomicExch(outside, 1);
            }
            double h = h_ref, v = v_ref;
            if (particle_time) {
                if (isnan(time)) {
                    h = time;
                    v = time; // match numpy.interp; validity check follows
        ...
        if (!bump_drift(xi, yi, zi, pxi, pyi, dpi, ti, ls, lt, inv_gamma2, after, s0, turn))
            break;
        s0 += after;
        t0 += after / velocity;
    }
