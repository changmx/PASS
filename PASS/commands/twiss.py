...
class Twiss(Command):
    ...
    def execute_gpu(self, sim):
        ...
        p = beam.particles
        real = p.real
        kernel = _get_twiss_kernel(p.dtype.str)
        threads = 256
        ...
        for bunch in beam.bunches:
            ...
            if n > 0:
                ...
                kernel(
                    (blocks, ),
                    (threads, ),
                    (p.x, p.px, p.y, p.py, p.z, p.dp, p.tag, np.int32(start), np.int32(end), real(m11_z), real(m12_z), real(m21_z), real(m22_z),
                     real(self.Dx_previous), real(self.Dpx_previous), real(self.Dx), real(self.Dpx), real(self.phi_x), real(self.phi_y), real(dq_x),
                     real(dq_y), real(sbx), real(bx_ratio), real(bx_prev_ratio), real(sby), real(by_ratio), real(by_prev_ratio), real(
                         self.alphax), real(self.alphax_previous), real(self.alphay), real(self.alphay_previous)),
                )
...
TWISS_KERNEL_BODY = r'''
extern "C" __global__ void transfer_twiss(
    ...
    pass_real_t dqy,
    pass_real_t sbx,
    pass_real_t bx_ratio,
    pass_real_t bx_prev_ratio,
    pass_real_t sby,
    pass_real_t by_ratio,
    pass_real_t by_prev_ratio,
    pass_real_t alphax,
    pass_real_t alphax_prev,
    pass_real_t alphay,
    pass_real_t alphay_prev
) {
    ...
    pass_real_t m21_x = (-(1 + alphax * alphax_prev) / sbx * sx + (alphax_prev - alphax) / sbx * cx);
    pass_real_t m22_x = bx_prev_ratio * (cx - alphax * sx);

    pass_real_t m11_y = by_ratio * (cy + alphay_prev * sy);
    pass_real_t m12_y = sby * sy;
    pass_real_t m21_y = (-(1 + alphay * alphay_prev) / sby * sy + (alphay_prev - alphay) / sby * cy);
...
def _get_twiss_kernel(dtype):
    try:
        ...
        raise RuntimeError("GPU Twiss tracking requires the optional 'cuda' dependencies.") from exc
