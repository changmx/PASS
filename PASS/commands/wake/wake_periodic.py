...
def prepare_periodic_slices(p,bunch,slices,turn=None,location=None):
    xp=p.xp
    if xp is not np:return prepare_periodic_gpu(p,bunch,slices,turn,location)
...
def prepare_periodic_gpu(p,bunch,slices,turn,location):
    ...
    if slices.explicit.z_max!=0. or not np.isclose(slices.explicit.z_min,-C,rtol=1e-13,atol=0):
        raise ValueError('Arrival-phase slices require an explicit [-C,0] interval')
    if not hasattr(slices,'observation_time'):raise ValueError('Use Slicer to define a common observation event')
    n=bunch.end_idx-bunch.start_idx
    history=getattr(slices,'_periodic_previous',{});samples=history.get(location,{})
    previous=samples.get(turn-1) if turn is not None else None
    use_previous=previous is not None and previous[1].shape==(n,)
    phase=cp.empty(n,dtype=cp.float64);alive=cp.empty(n,dtype=cp.int32);coordinate=cp.empty(n,dtype=p.z.dtype)
    ...
    if key not in cache:cache[key]=cp.RawKernel(_PERIODIC_CODE,'arrival_phase',
        options=('--std=c++17',f'-DFLOAT_PARTICLES={int(p.z.dtype==np.float32)}'))
    if n:cache[key]((min(256,(n+255)//256),),(256,),
        (p.z,p.tag,np.int64(bunch.start_idx),np.int64(n),np.float64(bunch.t0-slices.observation_time),
         np.float64(bunch.beta*const.c),np.float64(slices.observation_velocity/C),np.float64(C),
         previous[1] if use_previous else phase,previous[2] if use_previous else alive,np.int32(use_previous),
         phase,alive,coordinate,status))
    invalid,step=status.get()
    if invalid:raise ValueError('Live particle arrival phase must be finite')
    samples={key:value for key,value in samples.items() if turn is not None and key==turn-1}
    samples[turn]=(turn,phase,alive);history[location]=samples
    slices._periodic_previous=history;slices.periodic_max_step=float(step);slices._periodic_coordinate=coordinate
...
using R=double;
#endif
extern "C" __global__ void arrival_phase(const R* z,const int* tag,long long start,long long n,
    double epoch,double velocity,double factor,double circumference,const double* old,const int* old_alive,
    int previous,double* phase,int* alive,R* coordinate,double* status){
    double maximum=0.;int invalid=0;
