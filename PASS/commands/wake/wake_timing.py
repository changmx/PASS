...
class WakeClock:
    ...
    def sample(self):
        ...
        for b in self.beam.bunches:
            ...
            times[sl] = b.t0-p.z[sl].astype(xp.float64)/(b.beta*const.c)
        return times
    ...
    def slice_geometry(self, bunch, name, ids=None, alive=None):
        ...
        if xp is np:return epoch-centers/velocity, widths/velocity
        times,durations=xp.empty_like(centers),xp.empty_like(widths)
        if centers.size:
            self._kernel('slice_times',np.float64)(((centers.size+255)//256,),(256,),
...
using R=double;
#endif
extern "C" __global__ void sample_times(const R* z,const long long* ranges,const double* parameters,double* out){
    int b=blockIdx.y;long long end=ranges[2*b+1];
...
extern "C" __global__ void slice_times(const double* centers,const double* widths,long long n,
    double epoch,double velocity,double* times,double* durations){
    long long i=(long long)blockIdx.x*blockDim.x+threadIdx.x;if(i<n){times[i]=epoch-centers[i]/velocity;durations[i]=widths[i]/velocity;}
}
'''
...
def prepare_wake_tracking(sim,sequences):
    ...
    commands = [cmd for seq in sequences for cmd in seq.cmds
                if cmd.cmd_type == 'WakeField' and cmd.is_enabled]
