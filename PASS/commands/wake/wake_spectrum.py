...
def _sinc_derivative_factor(x):
    ...
    return np.where(small, x * (1 / 3 - x * x / 30 + x**4 / 840 - x**6 / 45360), (np.sin(x) - x * np.cos(x)) / (safe * safe))
...
def _cin(x):
    ...
    return np.where(x < 0.01, -x * x / 4 + x**4 / 96 - x**6 / 4320 + x**8 / 322560, sici(safe)[1] - np.euler_gamma - np.log(safe))
...
class ImpedanceSpectrum:
    ...
    def __post_init__(self):
        ...
        f = self.frequencies
        if (len(f) < 2 or f[0] < 0 or np.any(np.diff(f) <= 0) or self.real.shape != f.shape or self.imag.shape != f.shape):
    ...
    def inverse(self, times):
        ...
        for begin in range(0, t.size, block):
            ...
            x = np.pi * tau * df
            integral = df * np.exp(2j * np.pi * tau * mid) * (mean * np.sinc(tau * df) + 1j * difference * _sinc_derivative_factor(x))
    ...
    def primitive(self, times):
        ...
        for begin in range(0, t.size, block):
            ...
            logarithmic = (_cin(k * b) - _cin(k * a) + 1j * (sici(k * b)[0] - sici(k * a)[0]))
...
class RationalWakeModel(WakeModel):
    ...
    def __init__(self, poles, residues, longitudinal=True, original_spectrum=None, fit_diagnostics=None):
        ...
        if (not len(self.poles) or self.residues.shape != self.poles.shape or np.any(self.poles.real == 0)):
        ...
        while remaining:
            ...
            if self.poles[i].imag == 0:
                ...
                pairs = [j for j in remaining if self.poles[j] == self.poles[i].conjugate() and self.residues[j] == self.residues[i].conjugate()]
                if not pairs:
    ...
    def _value(self, times, primitive=False):
        ...
        for pole, residue in zip(self.poles, self.residues):
            ...
            value = (residue * np.expm1(pole * local) / pole if primitive else residue * np.exp(pole * local))
            if primitive:
...
def fit_spectrum(spectrum, initial_poles, *, optimize_poles=True, max_evaluations=500, relative_floor=1e-3, tolerance=0.02):
    ...
    if (not len(poles) or np.any(poles.real == 0) or np.any(poles.imag < 0) or not 0 < relative_floor <= 1 or not tolerance > 0
    ...
    x0 = np.r_[np.log(np.abs(poles.real) / scale_f), np.log(poles.imag[complex_mask] / scale_f)]
    ...
    result = (least_squares(solve, x0, max_nfev=max_evaluations, ftol=1e-12, xtol=1e-12, gtol=1e-12, bounds=(x0 - 20,
                                                                                                             x0 + 20)) if optimize_poles else None)
    p, coefficients, fitted = solve(result.x if result is not None else x0, full=True)
    error = np.abs(fitted - target)
    diagnostics = {
        "method": "variable_projection",
        "evaluations": 1 if result is None else result.nfev,
    ...
    for pole in p:
        if pole.imag == 0:
            ...
            ci += 2
    return RationalWakeModel(expanded_poles, residues, spectrum.longitudinal, original_spectrum=spectrum, fit_diagnostics=diagnostics)
...
__device__ double wake_sinc(
    double x
) {
    return x == 0. ? 1. : sin(x) / x;
}
__device__ double wake_cin(
    double x
) {
    x = fabs(x);
    double q = x * x;
    if (x < .01)
        return q * (-.25 + q * (1. / 96. + q * (-1. / 4320. + q / 322560.)));
    double si, ci;
    xsf::sici(x, si, ci);
    return ci - 0.5772156649015328606 - log(x);
}
__device__ double response(
    double time,
    const double* d,
    ...
    if (CAUSAL && time < 0.)
        return 0.;
    double t = CAUSAL ? fmax(time, 0.) : time, out = 0.;
    ...
    for (int j = 0; j < NPOINTS - 1; j++) {
        ...
        if (!primitive) {
            ...
            C slope = (z1 - z0) / df;
            for (int k = 0; k < 16; k++) {
                double ff = mid + df * .5 * nodes[k], phase = M_PI * ff * t;
                value += weights[k] * (z0 + slope * (ff - a)) * t * exp(C(0., phase)) * wake_sinc(phase);
            }
            value *= df * .5;
        } else {
            double k = 2 * M_PI * t, sa, ca, sb, cb;
            xsf::sici(k * a, sa, ca);
            xsf::sici(k * b, sb, cb);
