...
class RoundWallImpedance:
    ...
    def __post_init__(self):
        values = (self.radius, self.conductivity, self.length, self.skin_depth_ratio_max, self.surface_impedance_ratio_max)
    ...
    def validity(self, frequencies, beta):
        ...
        return {
            "max_skin_depth_ratio": float(np.max(ratio, initial=0)),
            "max_surface_impedance_ratio": float(np.max(zeta, initial=0)),
            "beta": float(beta),
            "content": "finite_conductivity_correction",
            "kinematics": "steady_equal_velocity"
        }

    def impedance(self, frequencies, beta, longitudinal=True):
        frequency_values = np.asarray(frequencies, float)
        self.validity(frequency_values, beta)
        angular_frequency = 2 * np.pi * np.abs(frequency_values)
        pipe_radius = self.radius
        k = angular_frequency / (beta * const.c)
        inverse_gamma2 = (1 - beta) * (1 + beta)
        x = pipe_radius * k * np.sqrt(inverse_gamma2)
        scaled_i0 = i0e(x)
        ...
        zeta = (1 - 1j) * np.sqrt(angular_frequency * const.epsilon0 / (2 * self.conductivity))
        if longitudinal:
            cgs = 2j * zeta / (const.c * pipe_radius) * inverse_i0_squared / (1j + zeta * angular_frequency * pipe_radius / const.c * j)
        else:
            # Algebraic reduction of Eq. (20), including gamma -> infinity.
            # gamma**2*(2*j-1) otherwise loses every significant digit near x=0.
            gterm = np.where(small, (pipe_radius * k)**2 * (-1 / 8 + x * x / 48 - 11 * x**4 / 3072),
                             (2 * j - 1) / max(inverse_gamma2,
                                               np.finfo(float).tiny))
            numerator = zeta * (pipe_radius * zeta * k * j * inverse_gamma2 + 1j * beta * (j - 1))
            denominator = const.c * j * pipe_radius * pipe_radius * (zeta * j * j * (1 + x * x) + 1j * pipe_radius * k * beta *
                                                                     (zeta * zeta + 1) * j * (j - 1) + zeta * beta * beta * gterm)
        ...
        return np.where(frequency_values < 0, negative, positive)

    def spectrum(self, frequencies, beta, longitudinal=True):
        frequency_values = np.asarray(frequencies, float)
        values = self.impedance(frequency_values, beta, longitudinal)
        return ImpedanceSpectrum(frequency_values, values.real, values.imag, longitudinal)
