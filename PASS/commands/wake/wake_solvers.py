...
class FFTConvolutionSolver:
    ...
    def compatible(sources, targets):
        ...
        return (step > 0 and np.all(steps > 0) and np.allclose(steps, step, rtol=1e-9, atol=max(abs(step) * 1e-11, rounding))
...
def solve_wake_cpu(components, current, state, *, turn, solver="direct", memory_turns=0, memory_time=None, fft_plan=None):
    ...
    if recursive:
        ...
        ordered = WakeSources(current.times[order], current.widths[order], {
            key: value[order]
            for key, value in current.moments.items()
        }, current.betas[order])
        use_fft = solver == "fft"
        if use_fft:
...
def kernel_gpu(component, tau, width, point, memory_time):
    ...
    return response_gpu(component.model, component.longitudinal).evaluate(tau,
                                                                          width=None if point else width,
...
def direct_gpu(components, sources, targets, memory_time=None, target_betas=None):
    ...
    for ci, c in enumerate(components):
        ...
        for s in sources:
            if not len(s.times):
                continue
            arrays = [cp.ascontiguousarray(v, dtype=cp.float64) for v in (s.times, s.widths, s.moments[c.source_powers], s.betas)]
            kernel(
                ((len(targets) + 3) // 4, ), (128, ),
                (targets, *arrays, target_beta, velocity, np.int32(nv), np.int32(witness), response.data, np.int32(len(targets)),
                 np.int32(len(s.times)), np.float64(c.scale), np.float64(np.inf if memory_time is None else memory_time), np.int32(s.point), out[ci]))
    return out
    ...
    for causal in {c.model.causal for c in components}:
        ...
        if spectrum is None:
            ...
            for row, c in enumerate(selected):
                response = response_gpu(c.model, c.longitudinal)
                response.kernel('response_grid')(
                    ((n + offset + 255) // 256, ), (256, ),
        ...
        response = response_gpu(owner, components[0].longitudinal)
        response.kernel('fft_product',
                        _FFT_CODE)(((product.size + 255) // 256, ), (256, ),
...
_FFT_CODE = r'''
extern "C" __global__ void fft_geometry(
    const double* t,
    const double* w,
    int n,
    double* out
    ...
    if (width > 0. && fabs(width - step) <= fmax(fabs(step) * 1.e-11, rounding) + 1.e-9 * fabs(step))
        step = width;
    for (int i = k; i < n; i += 256) {
        if (!isfinite(t[i]) || !isfinite(w[i]) || w[i] < 0. || !(step > 0.) || fabs(w[i] - width) > fabs(step) * 1.e-12 + 1.e-10 * fabs(width) ||
            width > step * (1. + 1.e-10) + rounding)
            atomicExch(&invalid, 1);
        if (i > 0 && (!(t[i] > t[i - 1]) || fabs((t[i] - t[i - 1]) - step) > fmax(fabs(step) * 1.e-11, rounding) + 1.e-9 * fabs(step)))
            atomicExch(&invalid, 1);
    }
    __syncthreads();
    if (k == 0) {
...
extern "C" __global__ void fft_product(
    const C* source,
    const C* response,
    const int* channels,
    int nf,
    ...
    if (i < n)
        out[i] = response[i] * source[(long long)channels[i / nf] * nf + i % nf];
}
'''
...
def event_arrays_gpu(source, components, geometry=None):
    ...
    if geometry is not None and geometry[0]:
        ...
        response.kernel('regular_events',
                        _EVENT_CODE)(((n + 255) // 256, len(components)), (256, ),
                                     (source.times, source.widths, moments, np.int32(n), np.int32(source.point), times, centers, impulses, drives))
    ...
    response.kernel('event_deposit',
                    _EVENT_CODE)(((n + 255) // 256, len(components)), (256, ),
                                 (indices, source.widths, moments, np.int32(n), np.int32(len(times)), np.int32(source.point), impulses, drives))
    if not source.point:
        response.kernel('event_prefix', _EVENT_CODE)((len(components), ), (256, ), (drives, np.int32(len(times))))
    return times, indices[:n], impulses, drives


_EVENT_CODE = r'''
#include <cub/block/block_scan.cuh>
extern "C" __global__ void mode_geometry(
    const double* t,
    const double* w,
    int n,
    int point,
    ...
    int k = threadIdx.x, bad = 0;
    double first = INFINITY, last = -INFINITY, origin = t[0];
    for (int i = k; i < n; i += 256) {
        double u = t[i] - origin, h = point ? 0. : w[i] * .5, left = u - h, right = u + h;
        first = fmin(first, left);
        last = fmax(last, right);
        if (!isfinite(u) || !isfinite(w[i]) || w[i] < 0.)
            bad = 1;
        if (!point && w[i] > 0. && !(left < u && right > u))
            bad = 1;
        if (i) {
            double prev = t[i - 1] - origin, prev_right = prev + (point ? 0. : w[i - 1] * .5);
            if (!(u > prev) || (!point && (!(left > prev) || !(prev_right < u))))
                bad = 1;
        }
    }
    int irregular = __syncthreads_or(bad);
    lo[k] = first;
    hi[k] = last;
    __syncthreads();
    for (int d = 128; d; d /= 2) {
        if (k < d) {
            lo[k] = fmin(lo[k], lo[k + d]);
            hi[k] = fmax(hi[k], hi[k + d]);
        }
        __syncthreads();
    }
    if (k == 0) {
...
extern "C" __global__ void regular_events(
    const double* t,
    const double* w,
    const double* q,
    int n,
    ...
    int i = blockIdx.x * blockDim.x + threadIdx.x, c = blockIdx.y;
    if (i >= n)
        return;
    ...
    if (point) {
        ...
        impulses[(long long)c * n + i] = charge;
        drives[(long long)c * n + i] = 0.;
        return;
    }
    int j = 3 * i, ne = 3 * n;
    double h = w[i] * .5, left = u - h, right = u + h;
    double rate = w[i] > 0. ? charge / w[i] : 0., after = 0.;
    if (i) {
        double prev_right = (t[i - 1] - t[0]) + w[i - 1] * .5;
        left = fmax(left, prev_right);
    }
    if (i + 1 < n) {
        double next_left = (t[i + 1] - t[0]) - w[i + 1] * .5;
        if (next_left < right)
            after = rate + (w[i + 1] > 0. ? q[(long long)c * n + i + 1] / w[i + 1] : 0.);
    ...
    impulses[base] = 0.;
    impulses[base + 1] = w[i] > 0. ? 0. : charge;
    impulses[base + 2] = 0.;
    drives[base] = rate;
    drives[base + 1] = rate;
    drives[base + 2] = after;
}
extern "C" __global__ void event_times(
    const double* t,
    const double* w,
    int n,
    int point,
    ...
    if (i < n) {
        ...
        if (!point) {
            events[n + i] = u - w[i] * .5;
            events[2 * n + i] = u + w[i] * .5;
        }
    }
}
extern "C" __global__ void event_deposit(
    const long long* ids,
    const double* widths,
    ...
    if (point || w <= 0.)
        atomicAdd(impulses + (long long)c * ne + ids[i], q);
    else {
        double rate = q / w;
        atomicAdd(drives + (long long)c * ne + ids[n + i], rate);
        atomicAdd(drives + (long long)c * ne + ids[2 * n + i], -rate);
    }
}
extern "C" __global__ void event_prefix(
    double* data,
    int n
) {
    typedef cub::BlockScan<double, 256> Scan;
    __shared__ Scan::TempStorage temp;
    __shared__ double base;
    int j = threadIdx.x;
    if (j == 0)
        base = 0.;
    __syncthreads();
    data += (long long)blockIdx.x * n;
    for (int start = 0; start < n; start += 256) {
        double v = start + j < n ? data[start + j] : 0., out, aggregate;
        Scan(temp).InclusiveSum(v, out, aggregate);
        if (start + j < n)
            data[start + j] = base + out;
        __syncthreads();
        if (j == 0)
            base += aggregate;
        __syncthreads();
...
extern "C" __global__ void scan_gather(
    const C* modes,
    const double* real_output,
    const C* residues,
    const long long* centers,
    const double* beta,
    ...
    if (i >= nt)
        return;
    long long j = centers[i];
    double sum = 0.;
    if (modal) {
        for (int k = 0; k < nm; k++)
            sum += (residues[k] * modes[(long long)k * ne + j]).real();
    } else
        sum = real_output[j];
    out[i] = sum * scale * coupling(beta[i], velocity, nv, 1);
}
'''
    ...
    if solver in {"recursive", "modal"}:
        ...
        if solver == "fft":
            if current.grid is not None or current.increasing:
                ...
                result[:, order] = fft_gpu(components,
                                           current.ordered(order),
                                           memory_time,
                                           resources=None if fft_plan is None else fft_plan.resources)
            if old:
...
extern "C" __global__ void rlc_scan(
    const double* times,
    ...
    for (int start = 0; start < n; start += B) {
        ...
        if (i < n) {
            ...
            if (q[m] > .5) {
                ...
                c = e * (1 + exp(-2 * d * dt)) / 2;
                s = -e * expm1(-2 * d * dt) / (2 * d);
            }
            fu = (1 - c - alpha * s) / w;
            if (fmax(w, alpha) * dt < 1e-4)
                fu = w * dt * dt *
                     (.5 - alpha * dt / 3 + (4 * alpha * alpha - w * w) * dt * dt / 24 +
                      (4 * alpha * w * w - 8 * alpha * alpha * alpha) * dt * dt * dt / 120);
            drive = i > 0 ? drives[m * n + i - 1] : 0.;
            impulse = impulses[m * n + i];
        }
        a00[j] = c + alpha * s;
        a01[j] = w * s;
        a10[j] = -w * s;
        ...
        if (i < n)
            output[m * n + i] = plane[m] ? (v - impulse / 2) : u;
        __syncthreads();
        if (j == min(B, n - start) - 1) {
            base[0] = u;
            base[1] = v;
        }
        __syncthreads();
...
extern "C" __global__ void modal_scan(
    const double* times,
    const double* impulses,
    const double* drives,
    const C* poles,
    ...
    for (int start = 0; start < n; start += B) {
        ...
        if (i < n) {
            ...
            C z = p * dt;
            a = exp(z);
            C em1(expm1(z.real()) * cos(z.imag()) - 2 * sin(z.imag() / 2) * sin(z.imag() / 2), exp(z.real()) * sin(z.imag()));
            impulse = impulses[i];
...
def _get_scan_kernel(solver, device):
    import cupy as cp

    with cp.cuda.Device(device):
        return cp.RawKernel(_SCAN_CODE, "rlc_scan" if solver == "recursive" else "modal_scan", options=("--std=c++17", ))
...
def advance_gpu(components, source, state, solver):
    ...
    kernel = _get_scan_kernel(solver, cp.cuda.runtime.getDevice())
    ...
    if solver == "recursive":
        params = ([c.model.omega for c in components], [c.model.q for c in components], np.array([c.longitudinal for c in components],
                                                                                                 dtype=np.int32))
        ...
        kernel((nc, ), (128, ), (times, impulses, drives, omega, q, vectors, output, np.int32(n), previous, has_previous, plane))
        ...
        for ci, c in enumerate(components):
            ...
            previous_vector = state.mode_amplitudes.get(ci)
            vector = (cp.zeros(len(poles), dtype=cp.complex128) if previous_vector is None else previous_vector.copy())
            output = cp.empty((len(poles), n), dtype=cp.complex128)
            kernel((len(poles), ), (128, ), (times, impulses[ci], drives[ci], poles, vector, output, np.int32(n), previous, has_previous))
...
def scan_gather_gpu(component, source, centers, values, out, *, residues=None, scale=1.):
    ...
    modal = residues is not None
    n = len(out)
    if n:
        response.kernel('scan_gather', _EVENT_CODE)(
            ((n + 255) // 256, ), (256, ),
