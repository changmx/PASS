...
class WakeSources:
    ...
    def __post_init__(self):
        ...
        if (times.ndim != 1 or widths.shape != times.shape or np.any(widths < 0) or not np.all(np.isfinite(times))
                or not np.all(np.isfinite(widths))):
            raise ValueError("Wake source times/widths must be finite matching 1D arrays, widths >= 0")
        moments = {k: np.array(v, dtype=float, copy=True) for k, v in self.moments.items()}
...
class WakeState:
    ...
    def fork(self, *, copy_modes=True):
        ...
        return WakeState(deque(self.history), modes, self.last_time, self.last_turn, self.last_source_end, self.convolution)
    ...
    def state_dict(self):
        ...
        return {
            "history": [{
                "turn": turn,
                "times": host(s.times).tolist(),
                "widths": host(s.widths).tolist(),
                "betas": host(s.betas).tolist(),
                "moments": [{
                    "powers": list(k),
                    "values": host(v).tolist()
                } for k, v in s.moments.items()]
            } for turn, s in self.history],
            "modes": [{
                "component": k,
                "real": host(v).real.tolist(),
                "imag": host(v).imag.tolist(),
                "complex": v.dtype.kind == "c"
            } for k, v in self.mode_amplitudes.items()],
            "last_time":
            self.last_time,
            "last_turn":
            self.last_turn,
            ...
            "convolution": (self.convolution.state_dict() if hasattr(self.convolution, "state_dict") else self.convolution)
        }
    ...
    def from_state_dict(cls, data):
        ...
        for row in data["history"]:
            history.append(
                (row["turn"], WakeSources(row["times"], row["widths"], {tuple(m["powers"]): m["values"]
        ...
        import copy
        return cls(history, modes, data["last_time"], last_turn, data["last_source_end"], copy.deepcopy(data.get("convolution")))
...
class DeviceSources:
    ...
    def upload(cls, source):
        ...
        if isinstance(source, cls):
            ...
            normalized = cls(pack(source.times), pack(source.widths), {
                k: pack(v)
        ...
        rows = cp.asarray(np.stack((source.times, source.widths, source.betas, *(source.moments[k] for k in keys))), dtype=cp.float64)
        return cls(rows[0], rows[1], {k: rows[i + 3] for i, k in enumerate(keys)}, rows[2], bool(np.all(source.widths == 0)))

    def ordered(self, order):
        import cupy as cp
        keys = list(self.moments)
        rows = [self.times, self.widths, self.betas, *self.moments.values()]
        ordered_values = cp.empty((len(rows), len(order)), dtype=cp.float64)
        pointers = cp.asarray([v.data.ptr for v in rows], dtype=cp.uint64)
        order = cp.ascontiguousarray(order, dtype=cp.int64)
        if len(order):
            array_kernel(self, 'gather_rows')(((ordered_values.size + 255) // 256, ), (256, ),
                                              (pointers, order, np.int64(len(order)), np.int32(len(rows)), ordered_values))
        return DeviceSources(ordered_values[0], ordered_values[1], {
            k: ordered_values[i + 3]
            for i, k in enumerate(keys)
        }, ordered_values[2], self.point)
...
def array_kernel(owner, name):
    ...
    if key not in cache:
        cache[key] = cp.RawKernel(_ARRAY_CODE, name, options=('--std=c++17', ))
    return cache[key]
...
def finite_gpu(owner, values):
    ...
    if values.size <= 65536:
        ...
        array_kernel(owner, 'finite_values_block')((1, ), (256, ), (values, np.int64(values.size), invalid))
        return not int(invalid[0])
    invalid = cp.zeros(1, dtype=cp.int32)
    if values.size:
        array_kernel(owner, 'finite_values')((min(256, (values.size + 255) // 256), ), (256, ), (values, np.int64(values.size), invalid))
    return not int(invalid[0])
...
def source_metadata_gpu(owner, source):
    ...
    array_kernel(owner, 'source_metadata')(
        (1, ), (256, ), (source.times, source.widths, source.betas, moments, np.int32(len(source.moments)), np.int64(len(source.times)), result))
    return result.get()


def time_bounds_gpu(owner, source):
    import cupy as cp
    time_bounds = cp.empty(4, dtype=cp.float64)
    array_kernel(owner, 'time_bounds')((1, ), (256, ), (source.times, source.widths, np.int64(len(source.times)), time_bounds))
    return time_bounds.get()


def shifted_times_gpu(owner, times, shift):
    import cupy as cp
    shifted_times = cp.empty_like(times)
    if shifted_times.size:
        array_kernel(owner, 'shift_times')(((shifted_times.size + 255) // 256, ), (256, ),
                                           (times, np.float64(shift), np.int64(shifted_times.size), shifted_times))
    return shifted_times


def add_gpu(owner, target, value):
    if target.size:
        array_kernel(owner, 'add_values')(((target.size + 255) // 256, ), (256, ), (target, value, np.int64(target.size)))
    return target


_ARRAY_CODE = r'''
#define INFINITY __longlong_as_double(0x7ff0000000000000LL)
extern "C" __global__ void shift_times(
    const double* t,
    double shift,
    long long n,
    double* out
...
extern "C" __global__ void time_bounds(
    const double* t,
    const double* w,
    long long n,
    double* out
    ...
    int k = threadIdx.x;
    double lo = INFINITY, hi = -INFINITY, width = 0., end = -INFINITY;
    for (long long i = k; i < n; i += 256) {
        lo = fmin(lo, t[i]);
        hi = fmax(hi, t[i]);
        width = fmax(width, w[i]);
        end = fmax(end, t[i] + w[i] * .5);
    }
    s[0][k] = lo;
    s[1][k] = hi;
    s[2][k] = width;
    ...
    for (int step = 128; step; step /= 2) {
        if (k < step) {
            s[0][k] = fmin(s[0][k], s[0][k + step]);
            for (int row = 1; row < 4; row++)
                s[row][k] = fmax(s[row][k], s[row][k + step]);
        }
        __syncthreads();
    }
    if (k < 4)
...
extern "C" __global__ void finite_values(
    const double* a,
    long long n,
    int* invalid
) {
...
extern "C" __global__ void finite_values_block(
    const double* a,
    long long n,
    int* invalid
) {
...
extern "C" __global__ void add_values(
    double* a,
    const double* b,
    long long n
) {
...
extern "C" __global__ void gather_rows(
    ...
    long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n * nr)
        out[i] = ((const double*)ptr[i / n])[order[i % n]];
}
extern "C" __global__ void source_metadata(
    const double* t,
    const double* w,
    const double* beta,
    const unsigned long long* moments,
    int nm,
    long long n,
    double* result
) {
    __shared__ double shared[5][256];
    int k = threadIdx.x;
    double valid = 1., lo = INFINITY, hi = -INFINITY, start = INFINITY, end = -INFINITY;
    for (long long i = k; i < n; i += 256) {
        double b = beta[i], a = t[i] - w[i] * .5, z = t[i] + w[i] * .5;
        if (!isfinite(t[i]) || !isfinite(w[i]) || w[i] < 0. || !isfinite(b) || b <= 0. || b > 1.)
            valid = 0.;
        lo = fmin(lo, b);
        hi = fmax(hi, b);
        start = fmin(start, a);
        ...
        for (int j = 0; j < nm; j++)
            if (!isfinite(((const double*)moments[j])[i]))
                valid = 0.;
    }
    shared[0][k] = valid;
    shared[1][k] = lo;
    ...
    for (int offset = 128; offset; offset /= 2) {
        if (k < offset) {
            shared[0][k] = fmin(shared[0][k], shared[0][k + offset]);
            shared[1][k] = fmin(shared[1][k], shared[1][k + offset]);
            shared[2][k] = fmax(shared[2][k], shared[2][k + offset]);
            shared[3][k] = fmin(shared[3][k], shared[3][k + offset]);
            shared[4][k] = fmax(shared[4][k], shared[4][k + offset]);
        }
        __syncthreads();
    }
    if (k < 5)
