...
class GroupExecution:
    ...
    def __init__(self, config, components, backend):
        ...
        if config.solver == "partitioned_fft":
            self.convolution = PartitionedConvolution(components,
                                                      ConvolutionGrid(**config.convolution_grid.model_dump()),
                                                      config.memory_turns,
                                                      backend=backend,
            ...
            self.convolution = TimeConvolution(components,
                                               TimeGrid(**config.time_grid.model_dump()),
                                               config.memory_time,
                                               backend=backend,
                                               method=config.partition or "dyadic",
                                               max_workspace_mb=config.max_workspace_mb or 1024)
    ...
    def preview(self, source, state, turn):
        ...
        if cfg.source_shape == "point":
            if gpu:
                source = Sources(source.times, xp.zeros_like(source.widths), source.moments, source.betas, True, None if source.grid is None else
                                 (source.grid[0], 0.), source.increasing)
            else:
        ...
        if self.convolution is not None:
            ...
            if gpu:
                ...
                if len(source.times):
                    ...
                    if hi - lo + width > cfg.period:
                        raise ValueError('Periodic source domain exceeds one declared period')
                copies = [
                    Sources(shifted_times_gpu(self, source.times, j * cfg.period), source.widths, source.moments, source.betas, source.point)
                ...
                if len(source.times) and float(xp.ptp(source.times) + xp.max(source.widths)) > cfg.period:
                    raise ValueError("Periodic source domain exceeds one declared period")
                copies = [
                    Sources(source.times + j * cfg.period, source.widths, source.moments, source.betas)
                    for j in range(-cfg.periodic_images, cfg.periodic_images + 1)
                ]
            ...
            values, candidate = solve(components,
                                      source,
                                      state,
                                      turn=turn,
                                      solver=cfg.solver,
                                      memory_turns=0 if cfg.history == "none" else cfg.memory_turns,
                                      memory_time=cfg.memory_time,
