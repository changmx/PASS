...
from .convolution import ConvolutionGrid, ConvolutionState, HistoryUpdate, PartitionedConvolution, BlockPreviewTransaction, _array_state
...
def _subset(source, index, xp):
    if xp is np:
        return WakeSources(source.times[index], source.widths[index], {k: v[index] for k, v in source.moments.items()}, source.betas[index])
    if index.dtype.kind == 'b':
        index = xp.nonzero(index)[0]
...
def _join(a, b, xp):
    ...
    return cls(xp.concatenate((a.times, b.times)), xp.concatenate((a.widths, b.widths)),
               {k: xp.concatenate((a.moments[k], b.moments[k]))
                for k in b.moments}, xp.concatenate((a.betas, b.betas)))
...
class TimeConvolutionState:
    ...
    @property
    def nbytes(self):
        recent = 0 if self.recent is None else sum(v.nbytes
                                                   for v in (self.recent.times, self.recent.widths, self.recent.betas, *self.recent.moments.values()))
    ...
    def state_dict(self):
        ...
        recent = None if self.recent is None else {
            "times": host(self.recent.times).tolist(),
            "widths": host(self.recent.widths).tolist(),
            "betas": host(self.recent.betas).tolist(),
            "moments": [{
                "powers": list(k),
                "values": host(v).tolist()
            } for k, v in self.recent.moments.items()]
        }
        return {
            "kind": "physical_time",
    ...
    def restore(cls, data, plan):
        ...
        state = cls(plan, data["start_turn"])
        state.count = data["count"]
        state.origin, state.last_end = data["origin"], data["last_end"]
        for value in (state.origin, state.last_end):
            if value is not None and not np.isfinite(value):
                raise ValueError("Physical-time checkpoint has an invalid clock")
        if (state.last_end is not None and state.origin is None) or (plan.grid.origin is not None and state.origin != plan.grid.origin):
            raise ValueError("Physical-time checkpoint origin is inconsistent")
        state.blocks = ConvolutionState.restore(data["blocks"], plan.blocks)
        item = data["open"]
        ...
        state.open = plan.xp.asarray(value.reshape(shape))
        if data["recent"] is not None:
            r = data["recent"]
            state.recent = WakeSources(r["times"], r["widths"], {tuple(m["powers"]): m["values"] for m in r["moments"]}, r["betas"])
            if set(state.recent.moments) != {c.source_powers for c in plan.components}:
                raise ValueError("Physical-time checkpoint source moments are inconsistent")
            if plan.backend == "gpu":
                from .wake_state import DeviceSources
                state.recent = DeviceSources.upload(state.recent)
        cursor = (state.blocks.start_turn + state.blocks.count) * plan.grid.block_size
        if state.last_end is None:
            if cursor or shape[1] or state.recent is not None:
                raise ValueError("Physical-time checkpoint has buffers without a source clock")
        else:
            position = (state.last_end - state.origin) / plan.grid.step
            expected = max(state.blocks.start_turn, math.floor(position - 1) // plan.grid.block_size) * plan.grid.block_size
            if state.count == 0 or cursor != expected or shape[1] != math.ceil(position) + 2 - cursor:
                raise ValueError("Physical-time checkpoint clock and open block are inconsistent")
            if state.recent is None or not len(state.recent.times):
            ...
            tolerance = 32 * abs(np.spacing(state.last_end))
            if not bool(np.all((ends <= state.last_end + tolerance) & (ends >= state.last_end - 2 * plan.grid.step - tolerance))):
...
class TimeHistoryUpdate:
    ...
    def commit(self):
        s = self.state
        if (s.count != self.count or s.blocks is not self.original_blocks or s.blocks.count != self.block_count):
...
class TimeConvolution:
    ...
    def __init__(self, components, grid, memory_time, *, backend="cpu", method="dyadic", max_workspace_mb=1024):
        ...
        self.blocks = PartitionedConvolution(components,
                                             ConvolutionGrid(period, 1, grid.block_size, period, grid.step),
                                             history,
                                             backend=backend,
                                             method=method,
        ...
        key = {"version": 2, "blocks": self.blocks.key, "origin": grid.origin, "projection": "integrated_hat_with_causal_correction"}
        ...
        state_bytes = 16 * math.prod(
            self.blocks.frequency_shape) * (self.blocks.input_capacity * len(self.blocks.channels) + self.blocks.pending_capacity * len(components))
    ...
    def _deposit(self, source, origin, start, size):
        ...
        deposited_moments = xp.zeros((len(self.blocks.channels), size), dtype=np.float64)
        if not len(source.times):
            return deposited_moments
        if self.backend == "gpu":
            from .wake_components import moments_gpu
            moments = moments_gpu(self.blocks.channels, source)
            deposit_gpu(source.times, source.widths, moments, deposited_moments, origin=origin, step=dt, start=start)
            return deposited_moments
        ...
        fraction = positions[point] - left
        for ci in range(len(moments)):
            np.add.at(deposited_moments[ci], left, moments[ci, point] * (1 - fraction))
            np.add.at(deposited_moments[ci], left + 1, moments[ci, point] * fraction)
        for i in np.flatnonzero(~point):
            left_edge, right_edge = positions[i] - widths[i] / 2, positions[i] + widths[i] / 2
            nodes = np.arange(math.floor(left_edge), math.ceil(right_edge) + 1)

            def hat(x):
                return np.where(x <= -1, 0., np.where(x < 0, .5 * (x + 1)**2, np.where(x < 1, 1 - .5 * (1 - x)**2, 1.)))
            ...
            if widths[i] < 1:
                ...
                weights = (hat(right_edge - nodes) - hat(left_edge - nodes)) / widths[i]
            keep = (nodes >= 0) & (nodes < size)
            nodes, weights = nodes[keep], weights[keep]
            deposited_moments[:, nodes] += moments[:, i, None] * weights
        return deposited_moments

    def _sample(self, ci, x, integral=False):
        xp = self.xp
        response, primitive = self.response[ci], self.primitive[ci]
        k = xp.floor(x).astype(np.int64)
        fraction = x - k
        j = xp.clip(k, 0, len(response) - 2)
        value = response[j] + fraction * (response[j + 1] - response[j])
        if integral:
            value = primitive[j] + fraction * response[j] + .5 * fraction * fraction * (response[j + 1] - response[j])
            return xp.where(x < -1, 0., xp.where(x < 0, .5 * response[0] * (x + 1)**2, xp.where(x >= len(response) - 1, primitive[-1], value)))
        return xp.where(x < -1, 0., xp.where(x < 0, response[0] * (x + 1), xp.where(x >= len(response) - 1, 0., value)))
    ...
    def _near_correction(self, source, recent, origin, frame_bytes=0, *, joined=None):
        ...
        moments = [self._coupled(component, near) for component in self.components]
        ...
        for begin in range(0, len(source.times), 256):
            ...
            pair_count = int(xp.sum(counts))
            if pair_count == 0:
                continue
            if self.estimated_bytes + frame_bytes + pair_count * 160 > self.budget:
                raise MemoryError("Physical-time near correction exceeds workspace; reduce source overlap or slice width")
            target_indices = xp.repeat(xp.arange(begin, end), counts)
            starts = xp.cumsum(counts) - counts
            source_indices = xp.repeat(lo[begin:end] - starts, counts) + xp.arange(pair_count)
            tau, width = source.times[target_indices] - near.times[source_indices], near.widths[source_indices]
            keep = xp.abs(tau) <= width / 2 + 2 * dt
            target_indices, source_indices, tau, width = target_indices[keep], source_indices[keep], tau[keep], width[keep]
            target = (source.times[target_indices] - origin) / dt
            source_positions = (near.times[source_indices] - origin) / dt
            width_in_bins = width / dt
            for ci, component in enumerate(self.components):
                if self.backend == "gpu":
                    ...
                    approximate = projected_pair_gpu(target, source_positions, width_in_bins, self.response[ci], self.primitive[ci])
                    exact = (exact_table_pairs_gpu(component, tau, width, self.memory_time)
                             if isinstance(component.model, TabulatedWakeModel) else kernel(component, tau, width, False, self.memory_time))
                else:
                    approximate = self._projected_pair_cpu(ci, target, source_positions, width_in_bins)
                    exact = _kernel(component, tau, width, self.memory_time)
                contribution = (exact - approximate) * moments[ci][source_indices]
                result[ci] += xp.bincount(target_indices, weights=contribution, minlength=len(source.times))
        return result
    ...
    def preview(self, source, state, *, turn):
        ...
        if self.backend == "gpu":
            ...
            if any(v.dtype != np.float64 or not v.flags.c_contiguous for v in arrays):
                source = DeviceSources(xp.ascontiguousarray(source.times, dtype=np.float64), xp.ascontiguousarray(source.widths, dtype=np.float64), {
                    k: xp.ascontiguousarray(v, dtype=np.float64)
                    for k, v in source.moments.items()
        ...
        if powers != set(source.moments):
            source = type(source)(source.times, source.widths, {k: source.moments[k] for k in powers}, source.betas)
        if state is None:
        ...
        if not len(source.times):
            return xp.zeros((len(self.components), 0)), TimeHistoryUpdate(state, (), state.open, state.recent, state.origin, state.last_end)
        ...
        if self.backend == 'gpu':
            ...
            valid = xp.all(
                xp.isfinite(source.times) & xp.isfinite(source.widths) & (source.widths >= 0)
            ...
            metadata = xp.stack((valid, xp.min(source.betas), xp.max(source.betas), xp.min(source.times - source.widths / 2),
                                 xp.max(source.times + source.widths / 2)))
        ...
        if self.backend == 'gpu':
            if opened_before.size:
                ...
                response.kernel('add_open_block', _CODE + _TIME_RESPONSE_CODE)(
                    ((opened_before.size + 255) // 256, ), (256, ),
        ...
        if self.backend == 'gpu':
            ...
            from .wake_models import response_gpu
            response = response_gpu(self.components[0].model, self.components[0].longitudinal)
            response.kernel('recent_mask', _CODE + _TIME_RESPONSE_CODE)(
                ((len(mask) + 255) // 256, ), (256, ), (joined.times, joined.widths, np.int64(len(mask)), np.float64(last - 2 * dt), mask))
    ...
    def diagnostics(self):
        return {
            **self.blocks.diagnostics, "time_step_s": self.grid.step,
            "time_block_size": self.grid.block_size,
            "history_horizon_s": self.memory_time,
            "estimated_peak_bytes": self.estimated_bytes,
            "projection": "integrated_hat",
            "causal_near_correction": True
        }
...
__device__ double tri(
    double x
) {
    return fmax(1. - fabs(x), 0.);
}
extern "C" __global__ void deposit_time(
    ...
    if (w == 0.) {
        ...
        for (long long j = left; j <= right; j++) {
            ...
            if (w < 1.) {
                ...
                weight = fabs(k - x) < half ? .5 * (tri(x - half) + tri(k)) * alpha + .5 * (tri(k) + tri(x + half)) * (1. - alpha) : tri(x);
            }
            for (int c = 0; c < channels; c++)
    ...
    for (int j = 0; j < 2; j++) {
        ...
        if (w >= 1.) {
            ...
            if (fabs(k - x) < half) {
                ...
                v = .5 * (sample_time(h, p, nh, x - half, false) + mid) * alpha + .5 * (mid + sample_time(h, p, nh, x + half, false)) * (1. - alpha);
            }
        }
        value += (j ? f : 1. - f) * v;
    }
...
extern "C" __global__ void projected_pair(
    const double* target,
    const double* position,
    const double* width,
    const double* h,
    ...
    long long i = (long long)blockDim.x * blockIdx.x + threadIdx.x;
    if (i < n)
        result[i] = mesh_pair(target[i], position[i], width[i], h, p, nh);
}
__device__ double table_value(
    double t,
    const double* x,
    const double* y,
    const double* slope,
    ...
    if (t < 0.)
        return 0.;
    if (t > x[n - 1])
        return integral ? primitive[n - 1] : 0.;
    int lo = 0, hi = n - 1;
    while (hi - lo > 1) {
        int mid = (hi + lo) / 2;
        if (x[mid] <= t)
    ...
    double d = t - x[lo];
    if (integral)
        return primitive[lo] + d * (y[lo] + .5 * slope[lo] * d);
    return (y[lo] + slope[lo] * d) * (t == 0. ? .5 : 1.);
}
extern "C" __global__ void exact_table_pairs(
    const double* tau,
    const double* width,
    const double* x,
    const double* y,
    ...
    if (w > 0.) {
        ...
        value = t > horizon ? 0. : table_value(t, x, y, slope, primitive, n, false);
...
def _get_time_kernel(name, device):
    import cupy as cp

    with cp.cuda.Device(device):
        return cp.RawKernel(_CODE, name, options=("--std=c++17", ))


def deposit_gpu(position, widths, moments, density, *, origin=0., step=1., start=0):
    import cupy as cp
    kernel = _get_time_kernel("deposit_time", cp.cuda.runtime.getDevice())
    kernel(((len(position) + 255) // 256, ), (256, ), (position, widths, moments, density, cp.int64(len(position)), cp.int64(
        density.shape[1]), cp.int32(len(moments)), np.float64(origin), np.float64(step), np.int64(start)))
...
def projected_pair_gpu(target, position, width, response, primitive):
    ...
    if not len(target):
        return result
    kernel = _get_time_kernel("projected_pair", cp.cuda.runtime.getDevice())
    kernel(((len(target) + 255) // 256, ), (256, ),
...
def exact_table_pairs_gpu(component, tau, width, horizon):
    ...
    if not len(tau):
        return out
    kernel = _get_time_kernel("exact_table_pairs", cp.cuda.runtime.getDevice())
    kernel(((len(tau) + 255) // 256, ), (256, ),
           (tau, width, *arrays, out, cp.int64(len(tau)), cp.int32(len(model.times)), cp.float64(horizon), cp.float64(component.scale)))
    return out
...
def time_gather_gpu(plan, source, values, correction, origin, start):
    ...
    for ci, c in enumerate(plan.components):
        ...
        response.kernel('time_gather', _CODE + _TIME_RESPONSE_CODE)(
            ((len(source.times) + 255) // 256, ), (256, ),
            (source.times, source.betas, values[ci], correction[ci], velocity, np.int32(nv), np.int64(len(
                source.times)), np.float64(origin), np.float64(plan.grid.step), np.int64(start), result[ci]))
    return result
...
extern "C" __global__ void max_width(
    const double* w,
    long long n,
    double* out
) {
...
extern "C" __global__ void time_gather(
    const double* times,
    const double* beta,
    const double* field,
...
extern "C" __global__ void recent_mask(
    const double* t,
    const double* w,
    long long n,
    double cutoff,
    ...
    long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = t[i] + w[i] * .5 >= cutoff;
}
extern "C" __global__ void add_open_block(
    double* density,
    const double* open,
    long long stride,
    long long width,
    ...
    long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        density[(i / width) * stride + i % width] += open[i];
}
'''
