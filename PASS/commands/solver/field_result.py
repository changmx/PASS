...
class GPUFieldSolver:
    ...
    def _check_context(self):
        ...
        if (cp.cuda.runtime.getDevice() != self.device or cp.cuda.get_current_stream().ptr != self.stream.ptr):
            raise RuntimeError("GPU resources must be used on their creation device and stream")
    ...
    def _source(self, density, validate):
        ...
        if squeeze:
            source = source[None]
        if (source.ndim != 3 or source.shape[1:] != (self.geometry.ny, self.geometry.nx) or source.shape[0] < 1):
    ...
    def prepare(self, num_slices):
        ...
        if self._work is None or self._work["slices"] != num_slices:
            ...
            self._work = dict(
                slices=num_slices,
                phi=cp.zeros((num_slices, grid.ny, grid.nx), self.dtype),
                ex=cp.empty((num_slices, grid.ny, grid.nx), self.dtype),
                ey=cp.empty((num_slices, grid.ny, grid.nx), self.dtype),
            )
...
def launch_gpu_kernel(source, name, size, args, dtype):
    """Launch one of a solver's cached kernels without host-array transfers."""
    import cupy as cp

    if size:
        _gpu_module(source, np.dtype(dtype).str, cp.cuda.runtime.getDevice()).get_function(name)(((size + 255) // 256, ), (256, ), args)


_GRADIENT_CUDA = r"""
extern "C" __global__ void gradient(
    const T* phi,
    T* ex,
    T* ey,
    int nx,
