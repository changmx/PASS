...
from .field_result import FieldResult, GPUFieldSolver, _gpu_module, _launch_gpu
...
class GPUDSTRectangleSolver(GPUFieldSolver):
    def __init__(self, geometry, dtype="float64", *, implementation="auto"):
        ...
        if implementation not in ("auto", "cufft", "fused", "cufftdx"):
            raise ValueError(
                "DST implementation must be 'auto', 'cufft', 'fused', or 'cufftdx'"
            )
        ...
        if implementation not in ("auto", "cufft") and not self._fused_supported:
            raise ValueError(
                "fused DST requires power-of-two extensions from 8 to 2048"
            )
        self.twiddles = {}
        if implementation == "fused":
            for n in set(lengths):
                angle = -2 * np.pi * np.arange(n // 2) / n
                self.twiddles[n] = cp.asarray(
                    np.column_stack((np.cos(angle), np.sin(angle))), dtype=self.dtype
                )
        self.dx_kernels = {}
        if implementation == "cufftdx":
            self.dx_kernels = {
                n: _cufftdx_kernel(n, self.dtype.str, self.device) for n in set(lengths)
            }
        ...
        self.inv_lambda = cp.asarray(
            1 / (const.epsilon0 * reference.eigenvalues), dtype=self.dtype
        )
    ...
    def prepare(self, num_slices):
        ...
        ns = int(num_slices)
        if (
            self.requested_implementation == "auto"
            and self._fused_supported
            and (self._work is None or self._work["slices"] != ns)
        ):
            self._select_implementation(ns)
        super().prepare(ns)

    def _select_implementation(self, ns):
        ...
        g = self.geometry
        key = (self.device, self.dtype.str, g.nx, g.ny, ns)
        if key not in _DST_CHOICES:
            reference = GPUDSTRectangleSolver(g, self.dtype, implementation="cufft")
            candidate = None
            try:
                candidate = GPUDSTRectangleSolver(
                    g, self.dtype, implementation="cufftdx"
                )
            except (
                ImportError,
                RuntimeError,
                ...
                warnings.warn(
                    f"cuFFTDx unavailable; using cuFFT DST: {exc}", RuntimeWarning
                )
            try:
                scores = {}
                if candidate is not None:
                    iy, ix = cp.indices((g.ny, g.nx), dtype=self.dtype)
                    rho = cp.broadcast_to(
                        cp.exp(
                            -(
                                (ix / (g.nx - 1) - 0.43) ** 2
                    ...
                    for a, b in ((actual.ex, expected.ex), (actual.ey, expected.ey)):
                        error = float(cp.linalg.norm(a - b) / cp.linalg.norm(b))
                        if not np.isfinite(error) or error > (
                            2e-4 if self.dtype.itemsize == 4 else 5e-10
                        ):
                            raise RuntimeError(
                                f"cuFFTDx DST validation failed: relative L2={error}"
                            )
                    ...
                    for index in range(6):
                        ...
                        for solver in pair:
                            ...
                            samples[solver.implementation].append(
                                cp.cuda.get_elapsed_time(start, end)
                            )
                    scores = {
                        name: float(np.median(times)) for name, times in samples.items()
                    }
                choice = (
                    "cufftdx"
                    if scores and scores["cufftdx"] < 0.95 * scores["cufft"]
                    else "cufft"
                )
        ...
        if self.implementation == "cufftdx" and not self.dx_kernels:
            self.dx_kernels = {
                n: _cufftdx_kernel(n, self.dtype.str, self.device)
                for n in {2 * (g.nx - 1), 2 * (g.ny - 1)}
            }
    ...
    def _prepare(self, ns):
        ...
        w["a"] = cp.empty((ns, g.ny - 2, g.nx - 2), self.dtype)
        w["b"] = cp.empty((ns, g.nx - 2, g.ny - 2), self.dtype)
        if self.implementation != "cufft":
            return
        fft_type = cufft.CUFFT_R2C if self.dtype.itemsize == 4 else cufft.CUFFT_D2Z
        w["transforms"] = []
        for rows, n in ((g.ny - 2, g.nx - 2), (g.nx - 2, g.ny - 2)):
            w["transforms"].append(
                (
                    cp.empty((ns * rows, 2 * (n + 1)), self.dtype),
                    cp.empty((ns * rows, n + 2), self.complex_dtype),
                    cufft.Plan1d(2 * (n + 1), fft_type, ns * rows),
                )
            )
    ...
    def solve(self, density, *, compute_potential=True, validate=True, copy=True):
        ...
        if self.implementation != "cufft":
            for mode, rows, n, dst in (
                ...
                if self.implementation == "cufftdx":
                    ...
                    _gpu_module(_DST_CUDA, self.dtype.str, self.device).get_function(
                        f"dst_fused_{length}"
                    )(
                        (w["slices"] * rows,),
                        (256,),
        ...
        for step in range(4):
            axis = step % 2
            rows, n = (g.ny - 2, g.nx - 2) if axis == 0 else (g.nx - 2, g.ny - 2)
            ext, spectrum, plan = w["transforms"][axis]
            _launch_gpu(
                _DST_CUDA,
                "dst_pack",
                ext.size,
            ...
            dst = w["phi"] if step == 3 else w["b"] if axis == 0 else w["a"]
            size = w["slices"] * rows * n
            _launch_gpu(
                _DST_CUDA,
                "dst_extract",
                size,
...
def _cufftdx_kernel(length, dtype, device):
    ...
    try:
        ...
        raise RuntimeError(
            "cuFFTDx requires nvidia-mathdx; install the PASS [cuda] extra"
        ) from exc
    include = next(
        (
            Path(package.locate_file(p)).parent
            for p in package.files
            if str(p).replace("\\", "/").endswith("include/cufftdx.hpp")
        ),
        None,
    )
    if include is None:
...
extern "C" __global__ void dst_pack(const T* src, T* ext, int n, int rows,
    int full_grid, long long size) {
    long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= size) return;
    int length=2*(n+1), k=i%length;
    long long row=i/length;
    if (k==0 || k==n+1) { ext[i]=0; return; }
...
extern "C" __global__ void dst_extract(const T* spectrum, T* dst, const T* inv_lambda,
    int n, int rows, int full_grid, int divide, long long size, T norm) {
    long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
    if (i>=size) return;
    int col=i%n, row=(i/n)%rows;
    long long batch=i/(n*rows), out=(batch*n+col)*rows+row;
    T value = -spectrum[2*((i/n)*(n+2)+col+1)+1]*norm;
    if (divide) value *= inv_lambda[col*rows+row];
    if (full_grid) out=batch*(n+2)*(rows+2)+(col+1)*(rows+2)+row+1;
    dst[out]=value;
}

template<int LENGTH> __device__ void block_fft(T* re,T* im,const T* twiddle) {
    for(int span=2;span<=LENGTH;span*=2) {
        for(int k=threadIdx.x;k<LENGTH/2;k+=blockDim.x) {
            ...
            re[a]=ar+br;im[a]=ai+bi;re[b]=ar-br;im[b]=ai-bi;
        }
        __syncthreads();
    }
}
template<int LENGTH> __device__ void fused_dst(const T* src,T* dst,const T* inv_lambda,
    const T* twiddle,int rows,int mode,T* memory) {
    constexpr int n=LENGTH/2-1;
    constexpr int log_length=LENGTH==8?3:LENGTH==16?4:LENGTH==32?5:LENGTH==64?6:
                             LENGTH==128?7:LENGTH==256?8:LENGTH==512?9:LENGTH==1024?10:11;
    T* re=memory;T* im=memory+LENGTH;
    ...
    for(int k=threadIdx.x;k<LENGTH;k+=blockDim.x) {
        ...
        re[rev]=value;im[rev]=0;
    }
    __syncthreads();block_fft<LENGTH>(re,im,twiddle);
    ...
    if(mode==1) {
        ...
        for(int k=threadIdx.x;k<LENGTH;k+=blockDim.x) {
            int rev=__brev((unsigned)k)>>(32-log_length);
            re[rev]=next[k/256];im[rev]=0;
        }
        __syncthreads();block_fft<LENGTH>(re,im,twiddle);
    }
    for(int col=threadIdx.x;col<n;col+=blockDim.x) {
        long long index=mode==2 ? (long long)batch*(rows+2)*(n+2)+(row+1)*(n+2)+col+1 : ((long long)batch*n+col)*rows+row;
        dst[index]=-im[col+1]*norm;
    }
}
#define FUSED_DST_WRAPPER(LENGTH) \
extern "C" __global__ void dst_fused_##LENGTH(const T* src,T* dst,const T* inverse,const T* twiddle,int rows,int mode) { \
    extern __shared__ double storage[];fused_dst<LENGTH>(src,dst,inverse,twiddle,rows,mode,(T*)storage); }
FUSED_DST_WRAPPER(8)
...
#include <cufftdx.hpp>
using T=REAL;
using FFT=decltype(cufftdx::Size<LENGTH>()+cufftdx::Precision<T>()+
    cufftdx::Type<cufftdx::fft_type::c2c>()+cufftdx::Direction<cufftdx::fft_direction::forward>()+
    cufftdx::ElementsPerThread<8>()+cufftdx::FFTsPerBlock<1>()+cufftdx::SM<ARCH>()+cufftdx::Block());
static_assert(!FFT::requires_workspace,"This DST path requires a workspace-free block FFT");
using C=FFT::value_type;
extern "C" __global__ void traits(int* out) {
    out[0]=FFT::block_dim.x;
    out[1]=FFT::shared_memory_size>sizeof(C)*LENGTH?FFT::shared_memory_size:sizeof(C)*LENGTH;
}
extern "C" __global__ void dst_dx(const T* src,T* dst,const T* inv_lambda,int rows,int mode) {
    ...
    for(int k=threadIdx.x;k<LENGTH;k+=blockDim.x) {
        ...
        buffer[k].x=value;buffer[k].y=0;
    }
    __syncthreads();FFT().execute(buffer);__syncthreads();
    if(mode==1) {
        ...
        __syncthreads();
        for(int k=threadIdx.x,slot=0;k<LENGTH;k+=blockDim.x,++slot) {buffer[k].x=next[slot];buffer[k].y=0;}
        __syncthreads();FFT().execute(buffer);__syncthreads();
    }
    for(int col=threadIdx.x;col<n;col+=blockDim.x) {
