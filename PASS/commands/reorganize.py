...
class ReorganizeBunch(Command):
    ...
    def __init__(self, beam_id: int, sim: Simulation, **command_kwargs):
        ...
        if kwargs.get("new harmonic number") is None:
            raise ValueError(f"ReorganizeBunch {self.cmd_name}: 'New harmonic number' "
                             f"is required")
        self.new_harmonic: int = int(kwargs["new harmonic number"])
        if self.new_harmonic < 1:
            raise ValueError(f"new harmonic number must be >= 1 in {self.cmd_name}")
    ...
    def execute_cpu(self, sim: Simulation):
        ...
        if turn != self.start_turn:
            return False
        set_simple_logging()
        logger.info(f"[ReorganizeBunch] {self.cmd_name}: switching harmonic "
        ...
        for k, b in enumerate(beam.bunches):
            logger.info(f"[ReorganizeBunch] bunch{k}: harmonic_id={b.harmonic_id}, "
