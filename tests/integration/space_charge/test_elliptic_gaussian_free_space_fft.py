"""Elliptic Gaussian transverse-field validation for the FFT Green solver.

Run from the repository root, for example::

    python -m tests.integration.space_charge.test_elliptic_gaussian_free_space_fft simana
    python -m tests.integration.space_charge.test_elliptic_gaussian_free_space_fft ana
"""
...
def simulate(run_dir: Path = DEFAULT_RUN_DIR) -> Path:
    """Generate and track the elliptic Gaussian source."""
    return _simulate(CASE, run_dir.resolve())


def analyse(run_dir: Path = DEFAULT_RUN_DIR) -> dict:
    """Compare the saved field with the Bassetti-Erskine analytic field."""
...
def test_full_workflow(sc_workflow):
    ...
    sc_workflow("elliptic_gaussian_free_space_fft")
