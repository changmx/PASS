...
以下接口中的字段用于 ``PASS.para.schema.elements.DriftItem`` 配置类；
...
四极铁、六极铁、八极铁、一般多极铁、弯铁、快脉冲偏转磁铁及螺线管内部的直线漂移阶段，
...
有理化形式保留了增量的小量信息，但不能消除反复累加中的舍入误差，也不能消除两个
物理贡献几乎抵消时的相消误差。

正向传播的有效粒子采用 :math:`p_z=\sqrt{p_z^2}`，不对正的纵向动量设置人为下限。
无效粒子不参与坐标更新，已记录的首次损失信息保持不变。

计算开销与计时
~~~~~~~~~~~~~~~~

对 N 个粒子的计算量为 O(N)。实际耗时取决于粒子数、存储精度、切片数、
临时数组和内存访问，应使用目标硬件与实际输入进行测量。
GPU 计时需排除首次编译，并用同步或 CUDA 事件覆盖异步执行。
单个漂移内核的耗时不能代表包含空间电荷、监视器和数据传输的完整模拟。
