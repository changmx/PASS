...
本模块介绍 PASS 中的弯转磁铁元件 **SBend** ，用于模拟带电粒子在扇形弯转磁铁中的运动。弯转磁铁是加速器中最基本的弯转元件，通过均匀磁场使粒子轨道偏转。

PASS 中的弯转磁铁为 **厚元件** （ ``length > 0`` ），支持完整的非线性跟踪，包括边缘角效应、边缘场效应、以及多种辛积分方案。
...
以下接口中的字段用于 ``PASS.para.schema.elements.SBendItem`` 配置类；
...
零长度弯转磁铁，仅施加 :math:`K_{0L}` 薄透镜动量更新，无 body 跟踪，无边缘效应。

小角度数值稳定性
----------------

极坐标漂移包含水平曲率项 :math:`\rho(1-\cos\theta)`。
CPU 和 GPU 统一使用半角恒等式
:math:`1-\cos\theta=2\sin^2(\theta/2)`，无需按角度分支判断。
融合 GPU 内核将半角正弦与既有的全角正弦、余弦一起预计算，供各切片复用，
内部切片路径也采用同一公式。这样可避免小角度消去误差，
也无需除以 :math:`1+\cos\theta`。粒子存储精度、积分设置和物理映射
保持不变。其他 FP32 舍入误差仍然存在，跟踪精度仍需通过精度和
切片收敛检查确认。

对于没有内部空间电荷节点的静态 rot-kick-rot 映射，FP32 粒子数组在 CPU 上使用
FP64 工作坐标，在融合 GPU 内核中使用 FP64 寄存器和标量参数。
内部旋转与二极场动量更新需要抵消参考弯转量；若每个子步都舍入到 FP32，可能掩盖很小的
...
一个完整的弯转磁铁由 **入口边缘** 、 **主体** 和 **出口边缘** 三部分组成，跟踪顺序为：
...
即 **一个真实入口、N 个本体切片、一个真实出口**。CPU 和 GPU 跟踪都在
本体循环之外施加入出口映射。内部空间电荷动量更新也只分割本体传输，不产生额外的
...
**Wedge 的作用** ：在均匀磁场 :math:`B_0` 中旋转观测平面，同时施加边缘角聚焦动量更新。Wedge 包含几何旋转（将坐标系转回参考轨迹系）和磁场动量更新两部分。
...
其中 :math:`H_D` 是精确直漂移哈密顿量（保留 :math:`p_z` 的根号，不做小动量展开），后三项是薄透镜动量更新。
...
.. note::
  ...
  "exact" 的含义：漂移部分保留精确根号 :math:`p_z = \sqrt{(1+\delta)^2 - p_x^2 - p_y^2}` ，不做 :math:`p_x \ll 1` 的小动量展开。近似仅在于将传播部分与动量更新部分分离（分裂算符法）。

二极动量更新映射 K
~~~~~~~~~~~~~~~~~~~~

动量更新部分合并为单个薄透镜 动量更新（位置不变，仅动量跳变）：
...
.. note::
  ...
  对于扇形弯铁 :math:`h = K_0` ，参考粒子（ :math:`x=0, \delta=0, \chi=1` ）的净动量更新为 :math:`h L_K - K_0 L_K = 0` 。这是正确的：在曲线坐标系中，参考粒子沿参考轨道运动， :math:`p_x` 始终为 0。弯转效应已编码在曲线坐标系本身中。
...
``model`` 可选 ``rot-kick-rot``、``drift-kick-drift-exact`` 或 ``adaptive``；
``adaptive`` 当前选择 ``rot-kick-rot``。

- **RKR**：极坐标输运包含参考轨道曲率，配合偶极场的动量更新。
- **DKD-exact**：将直线精确漂移与曲率、磁场的动量更新分裂组合。

两种模型的子映射与分裂方式不同。应针对偏转角、束流振幅、动量范围及目标观测量，
增加切片数并检查轨迹、工作点和纵向输运是否收敛；不能由单个子映射的解析精确性
推断一个切片适合所有参数。``integrator="adaptive"`` 当前采用 ``uniform``，
不是自动误差控制。四阶积分在光滑外场中的优势不消除浮点舍入和孔径取样误差。
内部空间电荷次数由独立的 ``Space charge.Num kicks`` 决定。
...
::
  ...
  动量更新: 无                          动量更新: Δpx = K0·x·sin(e1) (聚焦)
  ─────────────────                 ──────────────────
...
纵向坐标 :math:`z=\zeta=\beta_b c(T_b-t_i)` 。旋转参考系后， :math:`\sin\theta \cdot x` 是横向位置在新纵向方向上的投影（因参考系旋转导致的额外路径长度），需转换为时间增量：
...
其中 :math:`\text{tfac} = -(1/\beta_0 + p_\tau)` 来自 :math:`z=\zeta=\beta_b c(T_b-t_i)` 中 :math:`t` 对 :math:`p_z` 的依赖。

**隐式方程**

边缘场映射不是简单的 动量更新（位置不变、动量跳变），而是一个 **隐式映射** 。原因是边缘场效应与粒子的 :math:`y` 坐标非线性耦合：粒子在穿过边缘场时， :math:`y` 坐标本身也在变化，因此力（依赖 :math:`y` ）和位移（依赖力）相互耦合。
...
2. **磁场动量更新** ：施加边缘角聚焦效应 :math:`\Delta p_x \propto -x`
...
.. math::
  ...
  p_x' = \underbrace{p_x \cos\theta + p_z \sin\theta}_{\text{几何旋转}} \;\underbrace{-\; b_1 x \sin\theta}_{\text{磁场动量更新}}

- **几何旋转部分** ： :math:`p_x \cos\theta + p_z \sin\theta` ，与 YRotation 的 :math:`\cos\theta \cdot p_x - \sin\theta \cdot p_z` 方向相反，互相抵消
- **磁场动量更新部分** ： :math:`-b_1 x \sin\theta = -K_0 \chi x \sin\theta` ，正比于 :math:`-x` ，即边缘角聚焦

对于入口（ :math:`\theta = -e_1` ），动量更新为：
...
2. :math:`z=\zeta=\beta_b c(T_b-t_i)` ，更新 :math:`\zeta` 需要时间： :math:`\Delta t = \Delta\ell / v = \Delta\ell / (\text{rvv} \cdot \beta_0 c)`
...
- **负号** ： :math:`z=\zeta=\beta_b c(T_b-t_i)` ，时间增加导致 :math:`\zeta` 减小
