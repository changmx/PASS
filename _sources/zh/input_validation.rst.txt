...
* 全局粒子种类、非零电荷、正的周长及过渡 gamma、圈数、后端、粒子精度、
  Timing 和 GPU 设备编号。
  显式规定时钟要求正频率，设计速度（回转频率乘环长）小于光速；嵌套错误可定位到
  时钟/RF 时间表或 WakeField 求解组、分量字段。
* ``Sequence.injection`` 且 :math:`S=0`、连续束团编号、分组数和唯一谐波 ID、
  正的动能、粒子数量、发射度、Twiss 参数、分布类型和动量/动能偏移互斥关系。
  同一束流的非空束团必须具有相同的“真实粒子数 / 宏粒子数”，空束团继承此固定权重。
  RF 谐波仍独立于 束团分组数，不要求相等或整数倍。
...
* 显式及元件内部 SC：方法/求解器/分布模型组合、成对网格范围、网格尺寸、
  ...
  解析方法不支持保存电势、计算后端兼容性，以及内部 SC 的厚元件要求。
...
几何参数须按 :doc:`element/elseparator` 的接口显式填写。
