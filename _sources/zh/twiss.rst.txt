...
Twiss 传输直接保存更新后的束团相对纵向坐标 :math:`z_{\mathrm{rel}}` ，不把它折叠到单个环周范围。这样可保留多圈滑相的累计信息。需要实验室方位时，应使用 :math:`z_{\mathrm{lab}}=z_{\mathrm{rel}}+z_{\mathrm{center}}` ，并由具体分析任务决定是否取模。
