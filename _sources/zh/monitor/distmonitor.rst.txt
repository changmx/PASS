...
CPU 直接从 NumPy 粒子数组写出。GPU 在选中圈数将选定输出字段（默认九个）
...
Include injection metadata 默认 false，开启后增加 particle_id、injection_turn、
injection_batch，排序或损失后仍保留身份。Output format 支持默认 tfs 和 hdf5；
HDF5 使用压缩 dataset 保存相同数据列，用属性保存头信息。
不输出 tag=0 的预留位置，其数量记录为 NumPending。GPU 只复制选定输出字段。
