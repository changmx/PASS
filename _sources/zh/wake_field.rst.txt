...
粒子坐标保持连续的 ``z_rel``，``z_lab = z_rel + bunch.z_center``：
...
第二式用于薄元件处参考能量变化。通用 ``ArrivalClock`` 保存修正量，与尾场
是否启用无关，RFCavity 与 Exciter 使用相同修正。束团排序同时重排物理到达时间锚点；
重组后必须重新执行 Slicer。时钟不将折叠值写回 ``p.z``。

尾场时间适配器持有 ``beam.arrival_clock``，只管理尾场切片几何。
通用初始化、参考变换和采样时间存储属于 core，生命周期与检查点说明见
:doc:`arrival_time`。
...
``ArrivalClock.state_dict()`` 单独保存时钟修正。恢复粒子、参考参数、时钟与
尾场后，用 ``Executor.run(..., start_turn=...)`` 继续。仅恢复粒子数组不足以
恢复尾场；``reset_state()`` 将该位置场状态归零。
...
刚执行的等长 Slicer 且到达修正为零时，提供精确均匀网格并允许有限容量的
FFT 响应缓存。中间输运、网格改变或非零到达修正会使该快捷路径失效。这些
属于所选求解器的内部优化，不改变用户配置的物理算法。追踪时只同步少量
校验和控制元数据；检查点序列化明确将显存历史转换为可移植的主机数组。
...
CPU/CUDA 支持相同幂次。GPU 对普通项保留 Q/Qx/Qy 快速直方图，其余项使用
融合的通用单项式直方图；测试幂次在融合机械踢动核中按整数幂求值。
...
不按 bunch 枚举顺序推断。由于到达时间使用 -z_lab/(beta*c)，harmonic ID
递增不一定对应到达时间递增。
...
相同源幂次、相同源速度耦合的分量共用前向空间 FFT。``GroupExecution``
持有核与 FFT 资源，``ConvolutionState`` 持有频谱环形缓冲和待施加场，资源
不放在物理分量对象里。CUDA 执行计划跨长调度间隔持有 cuFFT 句柄，时间变换
使用连续时间批次；uniform 频谱乘加融合，不分配整段历史临时乘积。
刚执行 Slicer 的固定网格分块组使用整列投影与踢动核，多束团时钟也批量求值，
全程保留 float64。各组先预计算新场，再由命令施加踢动并提交历史；每圈
不复制完整历史频谱。可移植检查点包含采样核/网格指纹，在选定后端恢复，
恢复追踪前检查频谱形状与有限性。圈数必须连续，空圈也需执行。
新运行须 ``reset_state()``；初始过去历史为零。
...
对于持续滑移的演化漂移束，启用 Slicer 的 ``Periodic=true``、
``equal_length`` 和 ``Explicit={"z min": -C, "z max": 0}``，范围模式仍为
``explicit``。临时坐标为
:math:`C[((z_{rel}+z_{center}-\beta_0ca)/C\bmod1)-1]`，a 为到达时钟修正。
粒子存储坐标仍连续，束团分组不变。源切片依次覆盖 [t0,t0+T)，
T=C/(beta*c)，下一批从下一个物理参考通过时刻开始；使用 ``time_fft`` 时
周期可以变化。所有粒子群共享 t0、beta、C；Slicer 必须在 WakeField
同一位置。中间允许不改变到达相位及参考参数的薄横向元件和诊断；发生纵向
输运或参考参数变化后必须重新切片。演化历史使用 ``Boundary="causal_passages"``，另一个
重复稳态边界不能代替这种时间演化。
...
CUDA 把切片到达时钟统计合并为两遍中心化直方图。表格尾场近场精确修正
采用每个见证切片一个 warp，避免源—目标对列表及其计数的主机同步；其他
模型保留通用修正路径。性能测试包括逐束团 Slicer、参考参数变化、源投影、
...
WakeField 接受普通 ``Coordinate=z_rel`` 切片或漂移束
``Coordinate=arrival_phase`` 投影；旧 ``Periodic=true`` 仍选择后者。
``ring_position`` 会把不同过境投影到同一几何 bin，不能提供因果先后，因而
被拒绝。SpaceCharge 应使用另一个命名几何 SliceSet。

Coordinate 不改变算法组的 ``Boundary``。``Boundary=periodic`` 表示具有
周期镜像的空间响应，不能替代演化到达相位切片所需的 ``causal_passages``
边界及时间历史。core 到达修正仍用 FP64，连续 z 重分组时保持物理时间；
切片历史及冻结几何仍按各自的职责规则失效。
