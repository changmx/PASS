...
标量累计相位先在主机端使用高精度运算取一周期余数，再转换为 float64；
各粒子随后叠加围绕该参考事件的局部频率积分。这避免先将很大的
``frequency * elapsed_time`` 乘积舍入，或把微小粒子偏移加到很长的时间表区间上。
CPU 与 CUDA 使用同一缓存参考相位，粒子计算和存储保持上述精度。
该处理不能恢复输入 float64 参考时刻中已经丢失的时间信息。
