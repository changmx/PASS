...
若 :math:`p_z^2 \le 0`，粒子没有可供向前穿过元件的正纵向动量，标记为丢失。
...
代码中的 ``energy_over_gamma`` 对应 :math:`A`。
...
精确推导还假定计算 :math:`p_z` 时没有设置下限。CPU 当前在判断丢失后采用
:math:`p_z=\sqrt{\max(p_z^2,10^{-10})}`，GPU 则对有效粒子直接开平方。
所以在 :math:`0<p_z^2<10^{-10}` 时，两者可能出现偏差。这是修改前已有的边界处理，
需要与本次公式改写分别考虑。
