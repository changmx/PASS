...
坐标与时间的通用定义见 :ref:`zh-longitudinal-reference`。选择切片坐标和命名切片集后，在使用它的集体效应命令之前执行 Slicer。
...
进入对应边界切片并记录警告；不会静默丢弃。小于下限进入切片 N-1，
大于上限进入切片 0。

切片编号按所选坐标从大到小排列：切片 0 对应最大坐标区间，切片 N-1 对应最小坐标区间。
...
在束流初始化阶段，引用同一个 ``Slice set`` 的多个 ``Slicer`` 命令必须具有完全一致的 ``Slice model``、``Number of slices``、范围模式及（显式模式下的）显式参数块。配置冲突会抛出 ``ValueError``，并指出两个相关的序列命令。执行时，如果显式范围不能覆盖全部存活粒子，会记录警告；超出范围的粒子会被限制到第一个或最后一个切片。
...
如果存活粒子数小于 ``N``，最多只有这些粒子数目的切片能够非空。其余切片仍然保留，粒子数为 0，并记录警告。``effective_num_slices`` 保存 :math:`\min(N_{live},N)`；不会修改用户设置的网格大小。
...
超过总圈数的结束圈会截断到最后一圈并记录警告；起始圈大于等于总圈数的范围会
被忽略并记录警告。负起始圈也会截断，同时保持原始 ``start + k*step`` 的选圈
序列。``end < start``、非整数值和非正步长仍属于配置错误。

职责边界
--------

``SortBunch`` 和 ``ReorganizeBunch`` 负责全局束团分组，并且可能重排全部粒子数组。由于粒子范围可能改变，它们会使已有的 ``SliceSet`` 全部失效。随后由 ``Slicer`` 在序列指定位置重新计算目标集合。空间电荷和束束效应等需要切片的效应可以引用不同名称的集合，因此可以使用不同切片网格。
...
独立选项 ``arrival_phase`` 为非聚束束流尾场构造临时周期到达投影；
``Periodic=true`` 是该选项的别名。连续 z 加上名义分组位置不能重建物理环位置。

后续命令使用最近一次 Slicer 保存的 z 区间、宽度和成员，直到
...
``Z range mode`` 支持 ``auto`` 和 ``explicit``。粒子存储使用配置的 float32
...
使用该完整范围，并保留空切片；较窄范围外的折叠坐标进入边界切片。束团跨越折叠
边界时，``auto`` 范围可能覆盖接近整圈。

SpaceCharge 只接受该坐标。SC 配置要求对应 Slicer 显式设置
...
使用分别命名的切片集。``Periodic`` 标志选择 ``arrival_phase``；
...
边界下方的 FP32 坐标误分到相邻切片。CPU、CUDA 和 CUDA 批处理路径的分类与
