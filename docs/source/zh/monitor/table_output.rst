...
SpaceCharge 场输出保留现有多维 HDF5 结构，不增加 TFS 导出。
...
原有 TFS headers 保存为 **根目录 attributes**，包括名称、现有元数据中的
...
粒子分布、初始分布、相移及 Slicer 逐粒子快照直接将各列数组交给 HDF5
写入器，避免中间 DataFrame。分布回传限制在当前束团范围，Slicer
复用同一份 CPU tag 数组来统计计数和保存粒子表。
ParticleMonitor 保留完整轨迹缓存及一粒子一文件的组织方式。
...
统一读取器返回包含数据列与原 headers 的 ``TfsDataFrame``：
