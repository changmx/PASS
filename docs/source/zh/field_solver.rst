...
本页说明空间电荷场求解器的 Python 接口、数值模型和适用边界。若通过输入文件配置跟踪，
先阅读 :doc:`space_charge`；独立场计算则直接提供粒子横向坐标、切片编号、
宏粒子电荷和网格。批量网格数组按 ``(slice, y, x)`` 排列。

求解器返回纵向积分场，不处理作用长度、束流磁刚度或相对论修正因子；
这些量在 ``SpaceCharge`` 的横向动量更新中使用。
...
.. list-table::
   ...
   * - ``fft_free_space``
     - 开放自由空间
     - 仅完整网格，不允许导体孔径
     - 使用缓存 格林函数核的零填充 Hockney 线性卷积；适用于不考虑导体镜像
       电荷的情况。
...
.. list-table::
   ...
   * - ``build_pic_resources(...)``
     ...
     - 返回可复用 ``PICResources``，包括网格、孔径、有效掩码 和缓存求解器。
   * - ``deposit_particles(...)``
...
GPU 接口要求兼容的 CUDA 环境及 GPU 依赖；安装项目时选择 ``cuda`` 可选依赖。CPU 接口不要求 CuPy。
...
数值校验失败则报错。显式指定可选实现时，其依赖或编译错误直接报告。
融合实现支持 8 至 2048 点的二次幂延拓；其他合法尺寸使用 cuFFT，
不会修改配置的网格数。性能应在目标显卡和精度下测量。

``deposition_strategy`` 可取 ``atomic``、``warp`` 或 ``sorted_warp``；
后两种对同一节点的贡献合并求和，``sorted_warp`` 另对临时索引排序。
三者均保留粒子池顺序，采用相同的边界权重归一化与配对回插。

即使粒子采用 ``float32``，几何和孔径判断仍使用双精度中间量；
密度、电势、场及最终动量更新保留配置的精度。解析 GPU 接口
``solve_analytic_gpu`` 支持全部六种解析分布，中心矩及特殊函数的中间量使用
``float64``，粒子场采用配置精度。高精度中间计算不能恢复已在粒子存储中丢失的有效数字。
解析模型的诊断网格采样可在 CPU 上执行。

孔径接口
--------

SC 命令定义损失孔径，并在 FD/DST 中同时定义导体壁。
例如在命令内填写：
...
- 开放边界计算应增加网格范围，直到场对截断不敏感；所有求解器都应增加分辨率，
  直到场及横向动量变化等观测量收敛。
- CIC 更局部、计算量较小；TSC 使用更宽的插值模板，耦合更平滑。沉积与回插方法
  必须配对。
...
全部纵向切片作为前导右端项一起求解。几何掩码、稀疏矩阵分解、谱特征值或 FFT
...
沉积模板 会去除导体和边界节点，再逐粒子归一化剩余权重，从而守恒该粒子的
沉积电荷；回插过程使用相同的有效节点归一化。边界改变沉积模板 时会记录警告。
...
或没有有效的沉积节点，则报错。初始化校验和命令约定见 :doc:`space_charge`。
...
单位的平均场；``SpaceCharge`` 在计算横向动量增量之前用回插场除以该切片的 ``delta_z``。
...
网格线到物理壁交点的实际距离，因此物理边界不是简单的阶梯状节点掩码。
...
再单独计入相对论修正的横向动量增量。公式函数本身不包含 ``delta_z`` 或 1/gamma² 因子。
...
此表达式为连续泊松方程的场积分的闭式结果，适用于束内及束外，a 接近 b 时仍稳定。
...
``quasi-frozen`` 在每次空间电荷作用时重新计算各切片的总体矩：
...
用于跟踪时，以 ``Method="frozen"`` 或 ``"quasi-frozen"`` 配合对应公开
...
电势输出。采样网格不决定粒子横向动量增量，也不截断解析电荷分布。
