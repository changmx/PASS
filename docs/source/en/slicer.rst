...
See :ref:`en-longitudinal-reference` for coordinate and time conventions. Choose a coordinate and named slice set, then execute Slicer before each collective-effect command that needs its updated data.
...
SpaceCharge accepts only this coordinate. SC configuration requires ``"Coordinate": "z_periodic"`` in the corresponding Slicer. Missing coordinate metadata,
...
arrival times. Use separate named slice sets for SC and wakes. The
``Periodic`` flag selects ``arrival_phase``; ``Coordinate`` identifies
