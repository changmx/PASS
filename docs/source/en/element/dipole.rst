...
``Fint`` specifies the entrance fringe integral. A positive ``Fintx`` specifies
...
With :math:`N` internal body slices, this is:
...
There is **one real entrance, N body slices, and one real exit**. CPU and GPU
tracking apply the edge maps outside the body loop. Internal space-charge
kicks subdivide only body transport and do not create additional edge maps.
Increasing ``Num slices`` therefore requires no extra exit-integral parameter.
Representing a magnet as several independent ``SBend`` elements is a different
operation: each element then executes its own entrance and exit maps.
...
The fringe map uses the angle :math:`\Phi_0` below and the characteristic
coefficient :math:`\psi=b_0\tan\Phi_0`, where :math:`b_0=K_0\chi`:
...
- **Second term** :math:`-c_2(1 + x'^2(2+y'^2))p_z`: Fringe field integral effect. :math:`c_2` is the fringe field strength, and :math:`(1 + x'^2(2+y'^2))` is the higher-order slope correction.

**Partial derivatives and forces**

Differentiate :math:`\psi=b_0\tan\Phi_0` with respect to the independent
variables :math:`(x', y', p_z)`, then apply the chain rule to obtain the force
components. Differentiating the tangent supplies the factor
:math:`b_0/\cos^2\Phi_0`; the quantities below are derivatives of :math:`\psi`.
...
Here :math:`\phi_0=\Phi_0`, and :math:`\phi_1,\phi_2,\phi_3` are the partial
derivatives of :math:`\psi=b_0\tan\phi_0` with respect to :math:`x',y',p_z`.
