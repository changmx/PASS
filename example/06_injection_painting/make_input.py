...
from PASS.para.schema import MainConfig, Sequence, StatMonitor, DistMonitor, Slicer
from PASS.para.schema.bunch import BunchConfig, InjectionItem, OffsetConfig
from PASS.para.schema.elements import (DriftElement, SBendElement, QuadrupoleElement,
    MultipoleElement, SextupoleElement, BumpElement, RFCavityElement, ElSeparatorElement)
...
def dipole_fringe_integrals(name, values):
    ...
    if entrance > 0 and exit_ == 0:
        raise ValueError(
            f"Dipole {name}: fint1={entrance:g}, fint2={exit_:g} cannot be represented "
...
def build(source, output, *, turns=100, particles_per_batch=None, stage="aperture",
          clock="reference", backend="gpu", rf_directory=None, snapshots=True,
          grid_width=.5, grid_cells=None, es_length=1., particle_clock_origin="cisp",
          rf_voltage_unit=None, es_voltage=None, es_gap=None, es_vl=None):
    ...
    if physical_es:
        if (es_voltage is None) == (es_vl is None) or es_gap is None:
            raise ValueError("The physical ES requires exactly one of es_voltage (V) and es_vl (V m), plus es_gap (m); source septum cuts do not specify these values")
    ...
    offsets = {axis: OffsetConfig(is_offset=True,
        offset_position=number(inj[f"inj_pos_{axis}"]),
        offset_momentum=number(inj[f"inj_mom_{axis}"])) for axis in "xy"}
    bunch = BunchConfig(kinetic_energy=energy, num_macro_particles=per_batch*batch_count,
        num_real_particles=int(inj["particles_per_turn"])*batch_count*int(inj["ion_num"]),
        injection_turns=batch_count*int(inj["inj_interval"]), injection_interval=int(inj["inj_interval"]),
        is_load_from_file=True, file_path=str(distribution), file_mode="repeat",
        beta_x=number(inj["beta_x"]), beta_y=number(inj["beta_y"]),
        alpha_x=number(inj["alpha_x"]), alpha_y=number(inj["alpha_y"]),
        emit_x=number(inj["emit_x"]), emit_y=number(inj["emit_y"]),
        sigma_z=float(np.std(incoming[:, 4])), dp=float(np.std(incoming[:, 5])),
        offset_x=offsets["x"], offset_y=offsets["y"])
    ...
    if snapshots:
        seq.add("after_injection", DistMonitor(s=0., save_turns=selected_turns,
            include_injection_metadata=True, output_format="hdf5"))
    sc_on = stage == "pic"
    resource = definitions["sc01"][1]
    if sc_on:
        seq.add("slices", Slicer(s=0., slice_set="injection", num_slices=int(resource["slice_num_t"]),
            z_range_mode="explicit", explicit={"z min": -number(ring["circumference"])/2,
                                             "z max": number(ring["circumference"])/2}, coordinate="z_periodic"))
    ...
    for name in rotated:
        ...
        if v.get("switch", "on") == "off":
            if length:
                seq.add(name, DriftElement(**options, **slicing))
            continue
        if kind == "thickdrift":
            if physical_es and name in {"drift049a", "drift049b"} and s > es_start+1e-10:
                ...
                if original_start < es_start-1e-10:
                    ...
                    seq.add(name+"_approach", DriftElement(s=es_start, length=approach_length,
                        **aperture, **approach_slicing))
                    options["length"] = position-es_start
                    slicing["num_slices"] = source_slices(options["length"],max_slice)
                    if "space_charge" in slicing:
                        slicing["space_charge"] = ElementSpaceCharge(configuration="cisp", num_kicks=slicing["num_slices"])
                item = ElSeparatorElement(**options, **slicing, voltage=es_voltage, gap=es_gap,
                    voltage_length=None if es_vl is None else es_vl*(options["length"]/es_length),
                    tilt=theta-np.pi/2,
                    septum_position=outer-thickness, septum_thickness=thickness)
            else:
                item = DriftElement(**options, **slicing)
        elif kind == "thickquadrupole":
            item = QuadrupoleElement(**options, **slicing, k1l=number(v["k1"])*length, model="mat-kick-mat")
            ...
            item = SBendElement(**options, **slicing, integrator="yoshida4", k0l=number(v["angle"]), e1=number(v["e1"]),
                e2=number(v["e2"]), hgap=number(v["hgap"]), fint=fint, fintx=fintx)
        elif kind == "multipole":
            if stage == "external":
                continue
            item = MultipoleElement(**options, knl=[float(x) for x in v["knl"].split("|")[0].split("/")],
                ksl=[float(x) for x in v["ksl"].split("|")[0].split("/")])
        elif kind == "sextupole":
            item = SextupoleElement(**options, k2l=number(v["k2l"]))
        elif kind == "bump":
            ...
            item = BumpElement(**options, waveform_file=str(path), time_mode=clock, time_offset=time_offset)
            location_map[-1]["waveform_time_offset_s"] = time_offset
        elif kind == "paramonitor":
            item = StatMonitor(s=s)
        elif kind == "rf":
            ...
            for path, table in zip(files, tables):
                if (table.ndim != 2 or table.shape[1] != 2 or table.shape[0] < 2
                        or not np.all(np.isfinite(table)) or np.any(np.diff(table[:, 0]) <= 0)):
            ...
            program=synchronous_rf_program(values[:,1],values[:,2],harmonic,
                number(ring["circumference"]),mass,energy,int(inj["e_num"])/int(inj["a_num"]),
            ...
            item = RFCavityElement(s=s, components=[dict(program_file=str(rf_file))])
    ...
    main = MainConfig(beam_name="U238_35_painting", num_proton=92, num_neutron=146, num_electron=35,
        circumference=circumference, gamma_t=number(ring["gamma_t"]), num_turns=turns, backend=backend,
        output_dir=str(output/"tracking"))
    sc = SpaceChargeConfig(enabled=sc_on, configurations={"cisp": SpaceChargeResourceConfig(
        slice_set="injection", nx=2**int(resource["nx"]) if grid_cells is None else grid_cells,
        ny=2**int(resource["ny"]) if grid_cells is None else grid_cells,
        solver="fft_free_space", deposition_method="CIC", grid_width_x=grid_width, grid_width_y=grid_width)} if sc_on else {})
    input_path = generate_input(main, seq, str(output/"beam0.json"), space_charge=sc)
    manifest = dict(source=str(source), stage=stage, clock=clock, turns=turns, per_batch=per_batch,
        batches=batch_count, total_particles=per_batch*batch_count, source_files_sha256=provenance,
        location_map=location_map, rf_enabled=rf_directory is not None,
        ...
        source_length_m=source_length, final_s_rounding_m=circumference-source_length,
        pic_grid_width_m=grid_width, pic_grid_cells=sc.configurations["cisp"].nx if sc_on else None,
        source_particles_per_batch=int(inj["particles_per_turn"]),
        total_real_particles=bunch.num_real_particles,
        comparison_limits=["PASS uses its own external-element maps; CISP uses different chromatic/kinematic approximations.",
        ...
        electrostatic_separator=dict(enabled=physical_es, length_m=es_length if physical_es else None,
            voltage_V=es_voltage, integrated_voltage_V_m=es_vl, gap_m=es_gap,
            voltage_definition="septum potential minus counter-electrode potential", length_is_not_tilt_rescaled=True))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--particle-clock-origin", choices=["cisp", "injection"], default="cisp",
    ...
    parser.add_argument("--rf-voltage-unit", choices=["V", "MV"],
    ...
    es_strength.add_argument("--es-voltage", type=float, help="Septum minus high-voltage-electrode potential, V; choose this or --es-vl outside external stage")
    es_strength.add_argument("--es-vl", type=float, help="Total longitudinal integral of the ES voltage difference, V m; distributed by segment length")
    ...
    build(args.source, args.output, turns=args.turns, particles_per_batch=args.particles_per_batch,
        stage=args.stage, clock=args.clock, backend=args.backend, rf_directory=args.rf_directory,
        snapshots=not args.no_snapshots, rf_voltage_unit=args.rf_voltage_unit,
        es_voltage=args.es_voltage, es_gap=args.es_gap, es_vl=args.es_vl,
        grid_width=args.grid_width, grid_cells=args.grid_cells, es_length=args.es_length,
