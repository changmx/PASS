...
def run(path):
    ...
    sim = Simulation(cfg, [Beam(cfg.input_path[0], cfg)], State())
    ...
    result = {"completed_turns": cfg.num_turn, "seconds": time.perf_counter()-started,
        "alive": int((p.tag > 0).sum()), "lost": int((p.tag < 0).sum()),
        "pending": int((p.tag == 0).sum()), "output_directory": cfg.output_dir}
