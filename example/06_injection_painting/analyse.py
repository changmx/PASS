...
def write_gallery(output):
    ...
    collections = {"Injection evolution": [
        {"image": Path(path).name, "label": f"Turn {int(Path(path).stem.rsplit('_', 1)[1])}"}
        for path in manifest["snapshots"]]}
    for label, pattern in (("Injection batches", "injection_complete_batches_*.png"),
                           ("Full density", "injection_complete_density.png"),
                           ("Batch population", "batch_population_at_injection_complete.png"),
                           ("CISP comparison", "statistics_comparison_*.png")):
...
def analyse(directory, output=None, max_points_per_batch=2500, beam_id=0, bunch_id=0):
    ...
    for path in paths:
        ...
        counts.append({"turn": turn, "alive": len(alive), "lost": int((frame.tag < 0).sum()),
                       "injected": len(frame), "pending": int(metadata.get("NumPending", 0))})
    snapshots.sort()
    ...
    labels = {"x": "x (mm)", "y": "y (mm)", "px": r"$p_x=P_x/P_0$ ($10^{-3}$)",
              "py": r"$p_y=P_y/P_0$ ($10^{-3}$)"}
    summary = []
    ...
    def plot(frame, title, path, batches=None):
        ...
        for ax, (x, y) in zip(axes, panels):
            if batches is None:
                if len(alive):
                    ax.hist2d(alive[x]*1000, alive[y]*1000, bins=150,
                              range=[ranges[x], ranges[y]], cmap="viridis", cmin=1)
            else:
                ...
                for batch in batches:
                    ...
                    ax.scatter(part[x]*1000, part[y]*1000, s=3, alpha=.7,
                               color=palette(int(batch)%10), label=f"Batch {int(batch)+1} ({len(alive[alive.injection_batch == batch]):,})")
    ...
    for turn, path in snapshots:
        ...
        plot(frame, f"After injection point | turn {turn} | {int((frame.tag>0).sum()):,} survivors | newest batch {newest+1}",
             image, batches=[newest] if newest >= 0 else [])
    ...
    if completion is not None:
        ...
        population = frame.groupby("injection_batch").tag.agg(
            injected="size", alive=lambda values: int((values > 0).sum()),
        ...
        ax.set(xlabel="Injection batch (one-based)", ylabel="Macro-particles",
    ...
    (output/"plot_manifest.json").write_text(json.dumps({"snapshots": summary,
        "injection_complete_turn": completion[0] if completion else None,
        "batches": last_batch+1 if last_batch is not None else None,
        "scatter_sampling_only": True, "maximum_displayed_per_batch": max_points_per_batch}, indent=2), encoding="utf-8")
...
def compare_statistics(pass_csv, cisp_csv, output, note="Verify that the two runs use the same physical settings"):
    ...
    mapping = {"numAlive": ("particle_num", 1.), "xAverage": ("x_center(m)", 1.),
               "yAverage": ("y_center(m)", 1.), "xEmittance": ("emit_x(pi*m*rad)", 1.),
               "yEmittance": ("emit_y(pi*m*rad)", 1.)}
    # PASS reference energy is eV/u; CISP statistics report MeV/u.
    for key, other, factor in (("sigmaZ", "sigma_z(m)", 1.),
                               ("sigmadp", "sigma_dp", 1.),
                               ("Ek", "std_ke(MeV/u)", 1e6)):
    ...
    units = {"numAlive": (1e-6, "Survivors (million)"),
             "xAverage": (1e3, "Mean x (mm)"), "yAverage": (1e3, "Mean y (mm)"),
    ...
    for ax in list(axes.flat)[len(mapping):]:
        ax.axis("off")
    import textwrap
    axes.flat[-1].text(0, .9, textwrap.fill(note, 43), va="top", fontsize=10,
    ...
    output.with_suffix(".json").write_text(json.dumps({
        "PASS_file": str(Path(pass_csv).resolve()), "CISP_file": str(Path(cisp_csv).resolve()),
        "CISP_sha256": hashlib.sha256(Path(cisp_csv).read_bytes()).hexdigest(),
        "note": note, "matched_turns": len(result),
        "CISP_column_conversion": {key: {"column": other, "factor_to_PASS_units": factor}
                                   for key, (other, factor) in mapping.items()},
        "numerical_agreement_certified": False}, indent=2), encoding="utf-8")
