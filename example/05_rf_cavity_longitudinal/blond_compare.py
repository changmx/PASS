...
from make_input import CASES,CIRCUM,GAMMA_T,NUM_CHARGE,NUM_PROTON,NUM_NEUTRON,build_case,SCRIPT_DIR
...
def run_pass(case):
    ...
    if not report.ok:raise ValueError(report.text())
    cfg=Config();cfg.load_input(path);setup_logging(log_file=cfg.get_log_path())
    beam=Beam(path,cfg);sim=Simulation(cfg,[beam],State())
    seq=CommandSequence(cfg.input_data[0],0,sim);seq.sort()
...
def read_particles(output):
    ...
    for path in files:
        ...
        if missing:
            raise ValueError(
                f'{path}: missing reference columns {sorted(missing)}. '
                'Enable "Include reference": true in ParticleMonitor '
                '(make_input.py --include-reference) and rerun PASS before BLonD comparison.'
            )
        if np.any(table.tag<=0):raise ValueError(f'Comparison tag {tag} is lost or missing')
        result[tag]=table
    if set(result)!=set(TAGS):raise ValueError('Missing tagged monitor output')
    return result


def program_functions(path,origin=0.):
    """Independent analytic integral of the physical table, outside PASS tracker."""
    table=tfs.read(path);t=table.TIME.to_numpy();f=table.FREQUENCY.to_numpy()
    phi=table.PHASE.to_numpy();voltage=table.VOLTAGE.to_numpy()
    slopes=np.r_[np.diff(f)/np.diff(t),0.]
    phase_slopes=np.r_[np.diff(phi)/np.diff(t),0.]
    prefix=np.r_[0.,np.cumsum(np.diff(t)*(f[1:]+f[:-1])/2)]
    def primitive(time):
        j=int(np.clip(np.searchsorted(t,time,side='right')-1,0,len(t)-1));dx=time-t[j]
    ...
    def sample(time):
        ...
        return (float(np.interp(time,t,voltage)),
                2*np.pi*(primitive(time)-epoch)+float(np.interp(time,t,phi)),
                2*np.pi*float(np.interp(time,t,f))+modulation_slope)
    return sample


def compare(case,output,destination):
    native,backend_path=native_backend();data=read_particles(output)
    first=data[1];turns=len(first);A=NUM_PROTON+NUM_NEUTRON;mass=A*const.m_u_eV
    ...
    if not path.is_absolute():path=inputs[0].parent/path
    clock=raw.get('Reference clock') or {}
    sample=program_functions(path,float(clock.get('Time origin (s)',0.)))
    ref_p=A*float(first.referenceMomentum.iloc[0]);energy=np.hypot(ref_p,mass)
    beta=float(first.referenceBeta.iloc[0]);T=float(first.referenceTime.iloc[0])
    ...
    native_times=np.empty((turns,len(TAGS)));native_energy=np.empty_like(native_times)
    for n in range(turns):
        if n:
            V,phase,omega=sample(T);gain=NUM_CHARGE*V*np.sin(phase)
            native.kick(dt,dE,np.array([V]),np.array([omega]),np.array([phase]),NUM_CHARGE,1,-gain)
            energy+=gain;beta=np.sqrt((energy-mass)*(energy+mass))/energy
        native_times[n]=T+dt;native_energy[n]=energy+dE
        period=CIRCUM/(beta*const.c);eta=1/GAMMA_T**2-(mass/energy)**2
    ...
    time_error=native_times-measured_times;energy_error=native_energy-measured_energy
    metrics=[dict(tag=tag,max_time_difference_s=float(np.max(abs(time_error[:,j]))),
    ...
    for ax in axes.flat:ax.grid(alpha=.3);ax.set_xlabel('turn');ax.legend(fontsize=8)
    axes[0,0].set_ylabel('PASS arrival relative to reference / ns')
    axes[0,1].set_ylabel('BLonD arrival relative to PASS reference / ns')
    axes[1,0].set_ylabel('BLonD - PASS time / ps');axes[1,1].set_ylabel('BLonD - PASS energy / eV per ion')
    fig.suptitle(case+': declared map approximations');fig.tight_layout()
    image=io.BytesIO();fig.savefig(image,format='png',dpi=150);plt.close(fig)
    info=dict(case=case,turns=turns,initial_state='shared post-kick-0 state',blond_version=importlib.metadata.version('blond'),
              backend_source=str(backend_path),pass_output=str(output),rf_program=str(path),metrics=metrics)
    rows=''.join(f'<tr><td>{r["tag"]}</td><td>{r["max_time_difference_s"]:.6e}</td><td>{r["max_energy_difference_eV_per_ion"]:.6e}</td></tr>' for r in metrics)
...
<table><tr><th>tag</th><th>max time difference / s</th><th>max energy difference / eV per ion</th></tr>'''+rows+'</table><img src="data:image/png;base64,'+base64.b64encode(image.getvalue()).decode()+'"><pre>'+json.dumps(info,indent=2)+'</pre></html>'
    destination.mkdir(parents=True,exist_ok=True)
    report=destination/'pass_blond_report.html';report.write_text(html,encoding='utf-8')
    return report,info
...
def main():
    ...
    if output is None:parser.error('No complete output; run the PASS case first')
    report,info=compare(args.case,output,SCRIPT_DIR/'blond_comparison_output'/args.case)
    print(json.dumps(info,indent=2));print('Report:',report)
