...
int main(int argc, char** argv)
{
	try
	{
		...
		create_logger(Para.path_logfile.string());
		...
		logger->info("{:<30} {:d}h:{:d}min:{:d}s\n", "simulator running:", div(simulatorTime, 3600).quot, div(div(simulatorTime, 3600).rem, 60).quot,
					 div(simulatorTime, 60).rem);
	}
	...
	catch (...)
	{
		auto logger = spdlog::get("logger");
		if (logger)
			logger->critical("[FATAL] Unknown exception");
		else
